...
class Query implements PromiseLike<Row[]> {
  ...
  from(table: Table) { this.table = getTableName(table); return this; }
  innerJoin(..._args: unknown[]) { return this; }
  where(condition: SQL | undefined) { this.params = condition ? dialect.sqlToQuery(condition).params : []; return this; }
  orderBy(..._args: unknown[]) { return this; }
  limit(_count: number) { return this; }
  then<A = Row[], B = never>(resolve?: ((rows: Row[]) => A | PromiseLike<A>) | null, reject?: ((reason: unknown) => B | PromiseLike<B>) | null): Promise<A | B> {
...
class FakeDatabase {
  ...
  select(_columns: unknown) { return new Query(this); }
  async execute(query: Query): Promise<Row[]> {
    ...
    if (query.table === 'workspace_project') return [{id: projectId, title: 'Synthetic project', kind: 'marketing', status: 'active', updatedAt: new Date('2026-09-04T00:00:00Z')}];
    return [];
  }
...
async function main() {
  ...
  assert.equal(scoped.maximum, 4, 'Independent review, RFQ, lead, and publication queries must overlap');
  ...
  assert.ok(scoped.queries[0]?.params.includes(projectId), 'Project pages must scope the membership read');
  for (const query of scoped.queries.slice(1)) assert.ok(query.params.includes(projectId), 'Every task query must retain the authorized project filter');
  const denied = new FakeDatabase(false);
  assert.deepEqual(await listWorkspaceTasks(actorId, denied.asDatabase(), projectId), []);
  assert.equal(denied.queries.length, 1, 'No business records may be read without visible membership');
  ...
  console.log('PASS deterministic query concurrency: tasks 4-way, pipeline 2-way; project and actor authorization filters retained');
}
void main();
