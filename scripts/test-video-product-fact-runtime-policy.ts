...
const product: ProductReady = {
  ...
  product: { product_name: "Verified clutch kit", product_type: "clutch_kit", internal_sku: "FT-CL-551", oe_numbers: ["OE-551"] },
...
const project = videoProjectSchema.parse({
  ...
  platforms: ["facebook"],
  factualClaims: Object.entries(factValues).map(([field, value]) => ({ field, value, evidenceRef: fieldEvidence[field] })),
  sourceAssets: [{ assetRef: "evidence-source-551", mediaType: "image", rightsEvidenceRef: "evidence-rights-551" }],
  scenes: [{ sceneId: "scene-551", prompt: "Authorized product image", durationSeconds: 5, claimRefs: [], assetRefs: ["evidence-source-551"] }],
  approvalRefs: [],
  editDraft: {
    version: 2,
    platform: "facebook",
    clips: [{ clipId: "clip-551", assetRef: "evidence-source-551", mediaType: "image", trimStartMs: 0, durationMs: 5_000, fitMode: "contain", audioMode: "muted", caption: { kind: "verified_fact", claimRef: "product.oe_numbers" } }],
    ctaText: "Contact us",
  },
});
...
for (const [section, field, wrongValue] of [
  ...
  assert.throws(() => assertCurrentProductFacts(project, {
    ...product,
...
assert.throws(() => assertCurrentProductFacts(project, {
  ...product,
  field_evidence: { ...product.field_evidence, "product.oe_numbers": "evidence-replaced-oe-551" },
  evidence_refs: [...product.evidence_refs, "evidence-replaced-oe-551"],
}), /不是当前 ProductReady/);
assert.throws(() => assertCurrentProductFacts(project, { ...product, record_id: "00000000-0000-4000-8000-000000000599" }), /匹配的 ProductReady/);
