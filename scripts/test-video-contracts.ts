...
const project = {
  ...
  factualClaims: [
    { field: "product.name", value: "Synthetic clutch disc", evidenceRef: "evidence-product-001" },
  ],
  sourceAssets: [
    { assetRef: "asset-product-001", mediaType: "image", rightsEvidenceRef: "evidence-rights-001" },
  ],
  scenes: [
    {
      sceneId: "scene-001",
      prompt: "Show the approved product image.",
      durationSeconds: 5,
      claimRefs: ["product.name"],
      assetRefs: ["asset-product-001"],
    },
  ],
...
const privateTestProject = videoProjectSchema.parse({
  ...project,
  sourceAssets: [{ ...project.sourceAssets[0], usagePolicy: "private_test_only" }],
});
...
assert.throws(
  () =>
    videoProjectSchema.parse({
      ...project,
      scenes: [{ ...project.scenes[0], claimRefs: ["product.oe_number"] }],
    }),
  /未绑定证据/,
);
...
const event: WorkflowEventInput = {
  ...
  toState: "VIDEO_READY_FOR_GENERATION",
  actorType: "human",
  actorId: "creator-01",
  occurredAt: "2026-08-28T13:10:00Z",
...
assert.equal(
  videoCanvasDocumentSchema.parse({
    ...
    brief: {
      ...
      targetAudience: "Aftermarket distributors",
    },
...
const configuredCanvas = {
  ...
  nodes: [
    ...canvas.nodes,
    { id: "scene-demo", position: { x: 300, y: 300 }, data: { label: "Scene" } },
  ],
  ...
  platforms: ["youtube", "tiktok"],
  scenes: {
    "scene-demo": { prompt: "Show the rights-cleared product image.", durationSeconds: 5 },
  },
};
assert.equal(
  videoCanvasDocumentSchema.parse(configuredCanvas).scenes?.["scene-demo"]?.durationSeconds,
  5,
);
assert.equal(
  videoCanvasDocumentSchema.parse({
    ...configuredCanvas,
    nodes: [
      ...configuredCanvas.nodes,
      { id: "scene-later", position: { x: 400, y: 360 }, data: { label: "Later" } },
    ],
  }).nodes.length,
  6,
);
...
assert.throws(
  () =>
    videoCanvasDocumentSchema.parse({
      ...
      factBinding: {
        productId: "019a4c5c-1685-7f24-935b-2c27f7bbcc96",
        factPath: "engineering.oe_number",
      },
    }),
  /已核验/,
);
assert.throws(
  () =>
    videoCanvasDocumentSchema.parse({
      ...canvas,
      brief: { objective: "", targetAudience: "Aftermarket distributors" },
    }),
  /营销目标/,
);
assert.throws(
  () =>
    videoCanvasDocumentSchema.parse({
      ...canvas,
      scenes: { "scene-missing": { prompt: "Show the product.", durationSeconds: 5 } },
    }),
  /关联到画布/,
);
assert.throws(
  () =>
    videoCanvasDocumentSchema.parse({
      ...canvas,
      edges: [...canvas.edges, { id: "loop", source: "generate", target: "brief" }],
    }),
  /方向不符合|环路/,
);
assert.throws(
  () =>
    videoCanvasDocumentSchema.parse({
      ...canvas,
      edges: [...canvas.edges, { id: "duplicate", source: "facts", target: "generate" }],
    }),
  /只能有一条/,
);
assert.throws(
  () =>
    videoCanvasDocumentSchema.parse({
      ...
      edges: canvas.edges.map((edge) =>
        edge.id === "brief-facts" ? { ...edge, id: "other-edge" } : edge,
      ),
    }),
  /缺少必要连线/,
);
