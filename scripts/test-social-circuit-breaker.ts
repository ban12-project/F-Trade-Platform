...
import {
  applyWorkerObservation,
  resumeSocialChannel,
  type SocialChannelCircuitState,
} from "../lib/social/circuit-breaker";

const active: SocialChannelCircuitState = {
...
const paused = applyWorkerObservation(active, {
  channelRef: active.channelRef,
  accountRef: active.accountRef,
  externalEffectCertain: false,
});
assert.equal(paused.status, "paused");
assert.equal(paused.stopReason, "external_result_unknown");
assert.equal(
  applyWorkerObservation(paused, {
    channelRef: active.channelRef,
    accountRef: active.accountRef,
    stopReason: "captcha",
    externalEffectCertain: true,
  }),
  paused,
);
