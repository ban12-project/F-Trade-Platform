...
import {
  ...
  executeLeasedVideoJob,
  retryVideoJobAt,
  runNextConfiguredVideoJob,
...
async function main() {
  ...
  const project: VideoProject = {
    ...
    productId: "00000000-0000-4000-8000-000000000902",
    status: "approved",
    objective: "Synthetic objective",
    targetAudience: "Synthetic audience",
    platforms: ["tiktok"],
    factualClaims: [
      { field: "product.name", value: "Synthetic", evidenceRef: "evidence-product-901" },
    ],
    sourceAssets: [],
    scenes: [
      {
  ...
  const catalog = [
    {
      ...
      verificationRef: "evidence-provider-901",
      enabled: true,
    },
  ];
  const policies = [
    {
      provider: "google",
      enabled: true,
      credentialRef: "video-provider:google",
      maximumConcurrentJobs: 1,
      maximumAttempts: 3,
      budgetLimitCents: 1000,
      budgetCommittedCents: 200,
    },
  ];
  const result = await executeLeasedVideoJob(job, "google", {
    project,
    catalog,
    policies,
    adapters: [
      {
        ...
        async submit(request) {
          ...
          return { resultAssetRef: "asset-video-901" };
        },
      },
    ],
  ...
  assert.equal(
    classifyVideoJobFailure(new Error("retryable: temporary provider fault")),
    "provider_retryable",
  );
  ...
  console.log(
    "PASS video job runner executes a leased synthetic request and classifies retry behavior",
  );
}
