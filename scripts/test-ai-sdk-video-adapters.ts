...
const adapters = createAiSdkVideoAdapters(
  {
    async putGeneratedVideo() {
      return "asset-video-401";
    },
  },
  { googleVertex: { project: "synthetic-project", location: "us-central1" } },
);
assert.deepEqual(
  adapters.map((adapter) => adapter.provider),
  ["alibaba", "bytedance", "fal", "google", "google-vertex", "kling", "replicate", "xai"],
);
