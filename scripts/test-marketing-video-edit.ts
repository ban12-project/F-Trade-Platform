...
import {
  compileMarketingVideoAiDraft,
  type MarketingShotCandidate,
} from "../lib/video/shot-candidates";
...
const exact = marketingVideoDraftSchema.parse({
  version: 2,
  platform: "facebook",
  clips: [baseClip, { ...baseClip, clipId: "clip-002", durationMs: 5_000 }],
  ctaText: "Contact us",
});
const { caption: _caption, ...legacyBaseClip } = baseClip;
assert.equal(
  exact.clips.reduce((sum, clip) => sum + clip.durationMs, 0),
  15_000,
);
assert.throws(
  () =>
    marketingVideoDraftSchema.parse({
      ...exact,
      clips: [
        { ...exact.clips[0]!, durationMs: 10_000 },
        { ...exact.clips[1]!, durationMs: 5_001 },
      ],
    }),
  /15 秒/,
);
assert.throws(
  () =>
    marketingVideoDraftSchema.parse({
      ...exact,
      clips: [{ ...exact.clips[0]!, durationMs: 10_001 }],
    }),
  /10 秒/,
);
assert.throws(
  () =>
    marketingVideoDraftSchema.parse({
      ...exact,
      clips: [{ ...exact.clips[0]!, mediaType: "image", trimStartMs: 1 }],
    }),
  /图片素材/,
);
...
assert.deepEqual(legacy.clips[0]?.caption, {
  kind: "verified_fact",
  claimRef: "product.oe_number",
});
assert.throws(
  () =>
    marketingVideoDraftSchema.parse({
      ...
      clips: [
        {
          ...legacyBaseClip,
          subtitle: "Ambiguous facts",
          claimRefs: ["product.oe_number", "commercial.moq"],
        },
      ],
      ctaText: "Contact us",
    }),
...
assert.equal(
  marketingVideoAiDraftSchema.safeParse({
    clips: [
      { ...legacyBaseClip, subtitle: "Incorrect OE 99999", claimRefs: ["product.oe_number"] },
    ],
    ctaText: "Contact us",
  }).success,
  false,
...
assert.equal(
  marketingVideoAiDraftSchema.safeParse({
    clips: [
      {
        ...aiBaseClip,
        caption: { kind: "creative", text: "Built with ten splines", claimRef: "" },
      },
    ],
    ctaText: "Contact our sales team",
  }).success,
...
assert.equal(
  marketingVideoAiDraftSchema.safeParse({
    clips: [
      {
        ...aiBaseClip,
        caption: { kind: "creative", text: "For distributor inquiries", claimRef: "" },
      },
    ],
...
assert.equal(
  marketingVideoAiDraftSchema.safeParse({
    clips: [
      {
        ...aiBaseClip,
        caption: { kind: "verified_fact", text: "", claimRef: "product.oe_number" },
      },
    ],
...
const shotCandidates: MarketingShotCandidate[] = [
  {
    ...
    sourceAnalysis: {
      version: "ffmpeg-scdet-frame-diff-v1",
      intervalStartMs: 3_000,
      intervalEndMs: 7_000,
      representativeMs: 5_000,
      actionScore: 63,
      actionLevel: "high",
      startBoundary: "scene",
      endBoundary: "window",
...
const aiSuggestion = marketingVideoAiDraftSchema.parse({
  clips: [
    { ...aiBaseClip, caption: { kind: "verified_fact", text: "", claimRef: "product.oe_number" } },
  ],
  ctaText: "Contact our sales team",
});
const compiledAiDraft = compileMarketingVideoAiDraft({
...
assert.throws(
  () =>
    compileMarketingVideoAiDraft({
      suggestion: {
        ...aiSuggestion,
        clips: [{ ...aiSuggestion.clips[0]!, shotCandidateId: "shot-999-999" }],
      },
      candidates: shotCandidates,
      platform: "facebook",
    }),
...
assert.throws(
  () =>
    compileMarketingVideoAiDraft({
      suggestion: { ...aiSuggestion, clips: [{ ...aiSuggestion.clips[0]!, durationMs: 3_001 }] },
      candidates: shotCandidates,
      platform: "facebook",
    }),
  /可用时长/,
...
assert.throws(
  () =>
    compileMarketingVideoAiDraft({
      suggestion: { ...aiSuggestion, clips: [aiSuggestion.clips[0]!, aiSuggestion.clips[0]!] },
      candidates: shotCandidates,
      platform: "facebook",
    }),
  /重复选择/,
...
const project: VideoProject = {
  ...
  platforms: ["facebook"],
  factualClaims: [
    { field: "product.oe_number", value: "12345", evidenceRef: "evidence-product-501" },
  ],
  sourceAssets: [
    {
      assetRef: "evidence-source-001",
  ...
  scenes: [
    {
      sceneId: "scene-001",
      prompt: "Authorized source",
      durationSeconds: 10,
      claimRefs: [],
      assetRefs: ["evidence-source-001"],
    },
  ],
  approvalRefs: [],
  editDraft: marketingVideoDraftSchema.parse({
    version: 2,
    platform: "facebook",
    clips: [baseClip],
    ctaText: "Contact us",
...
void (async () => {
  ...
  assert.throws(
    () =>
      createMarketingEditTimeline(project, {
        ...project.editDraft!,
        clips: [
          {
            ...project.editDraft!.clips[0]!,
            caption: { kind: "verified_fact", claimRef: "specifications.diameter" },
          },
        ],
      }),
    /未绑定证据/,
      ...
      renderApprovedMarketingTimeline(
        {
          timeline: {
            ...timeline,
            durationSeconds: 16,
            scenes: [{ ...timeline.scenes[0]!, durationSeconds: 16 }],
          },
          platform: "facebook",
          width: 1080,
          height: 1920,
  ...
  console.log(
    "PASS MVP1 marketing edit compiles verified captions server-side and enforces 15 seconds",
  );
})();
