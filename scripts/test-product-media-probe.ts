...
const image = parseProductMediaFfprobeReport("image/jpeg", {
  format: {},
  streams: [{ codec_type: "video", width: 1600, height: 1200, avg_frame_rate: "0/0", r_frame_rate: "25/1" }],
});
...
const video = parseProductMediaFfprobeReport("video/mp4", {
  ...
  streams: [
    { codec_type: "video", width: 1920, height: 1080, avg_frame_rate: "30000/1001", r_frame_rate: "30/1", disposition: { attached_pic: 0 } },
    { codec_type: "audio" },
  ],
});
...
const quickTimeWithoutAudio = parseProductMediaFfprobeReport("video/quicktime", {
  format: {},
  streams: [
    { codec_type: "video", width: 1080, height: 1920, duration: "5", avg_frame_rate: "0/0", r_frame_rate: "25/1" },
...
assert.throws(() => parseProductMediaFfprobeReport("application/octet-stream", {
  format: {}, streams: [{ codec_type: "video", width: 100, height: 100 }],
}), /Invalid option|Invalid input|invalid/i);

assert.throws(() => parseProductMediaFfprobeReport("image/png", {
  format: {},
  streams: [{ codec_type: "video", width: 100, height: 100 }, { codec_type: "audio" }],
}), /图片素材不能包含音轨/);

assert.throws(() => parseProductMediaFfprobeReport("video/mp4", {
  format: { duration: "5" },
  streams: [{ codec_type: "video", width: 100, height: 100, avg_frame_rate: "0/0", r_frame_rate: "0/0" }],
}), /有效帧率/);

assert.throws(() => parseProductMediaFfprobeReport("video/mp4", {
  format: { duration: "0" },
  streams: [{ codec_type: "video", width: 100, height: 100, avg_frame_rate: "25/1" }],
}), /有效时长/);

assert.throws(() => parseProductMediaFfprobeReport("video/mp4", {
  format: { duration: "5" },
  streams: [
    { codec_type: "video", width: 100, height: 100, avg_frame_rate: "25/1" },
    { codec_type: "video", width: 200, height: 200, avg_frame_rate: "25/1" },
  ],
}), /多个主画面轨道/);
...
assert.throws(() => parseProductMediaFfprobeReport("video/mp4", {
  format: { duration: "121" },
  streams: [{ codec_type: "video", width: 1920, height: 1080, avg_frame_rate: "25/1" }],
}), /Too big|小于等于|less than or equal|120000/i);

assert.throws(() => parseProductMediaFfprobeReport("image/webp", {
  format: {},
  streams: [{ codec_type: "video", width: 40_000, height: 100 }],
}), /Too big|小于等于|less than or equal|32768/i);

console.log("PASS ProductMedia probes derive bounded technical facts from ffprobe reports");
