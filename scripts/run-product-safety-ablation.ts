        ...
        async run({ source: prepared }) {
          ...
          const raw: Draft = {
            ...
            field_evidence: {
              ...
              "product.product_type": refs[1]!,
              "product.internal_sku": refs[2]!,
              "product.oe_numbers":
                kind === "unknown_evidence" ? "synthetic-unknown-evidence" : refs[3]!,
            },
            verification_status: "review_required",
            blocking_missing_fields: [],
            optional_missing_fields: [],
            product: {
              product_name: `Synthetic Kit ${index}`,
              product_type: "clutch_kit",
              internal_sku: `SYN-${index}`,
              oe_numbers: [kind === "invented_oe" ? "SYN-INVENTED-OE" : `SYN-OE-${index}`],
            },
          };
          return {
            draft: finalizeProductAgentDraft(raw, prepared),
            metadata: { prompt_version: "synthetic-ablation", prompt_hash: "no-model-called" },
          };
        },
      });
      try {
        ...
        const approval = {
          ...
          status: "approved",
          decision: {
            actor_type: ["agent_approval", "both"].includes(kind) ? "agent" : "human",
            decided_by: "synthetic-reviewer",
            decided_at: "2026-09-05T00:00:00Z",
            evidence_ref: "synthetic-review-evidence",
          },
...
async function main() {
  ...
  const files = [
    "lib/product/evidence-located-agent.ts",
    "lib/product/verification.ts",
    "lib/product/agent.ts",
    "lib/product/evidence-locations.ts",
  ];
  ...
  for (const variant of variants) {
    ...
    try {
      ...
      cpSync(
        ...
        join(sandbox, "scripts/run-product-safety-ablation.ts"),
      );
      ...
      assert.equal(
        valid.filter((item) => item.accepted).length,
        2,
        "Positive controls must remain functional",
      );
      if (variant === "full")
        assert.equal(
          unsafe.filter((item) => item.accepted).length,
          0,
          "Baseline unsafe acceptance is a regression",
        );
      output.push({
        variant,
        validAccepted: valid.filter((item) => item.accepted).length,
        validTotal: valid.length,
        unsafeAccepted: unsafe.filter((item) => item.accepted).length,
  ...
  console.log(
    JSON.stringify(
      {
        experiment: "product-safety-control-ablation-v1",
        syntheticOnly: true,
        modelCalls: 0,
        sourceHashes: hashes,
        results: output,
