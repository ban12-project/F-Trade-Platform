  ...
  clips: [
    {
      ...
      fitMode: "cover",
      audioMode: "muted",
      caption: { kind: "verified_fact", claimRef: "product.product_name" },
    },
  ],
...
assert.deepEqual(migrated.clips[0]?.abcdRoles, [
  "attention",
  "branding",
  "connection",
  "direction",
...
const incomplete = {
  ...migrated,
  clips: [{ ...migrated.clips[0]!, abcdRoles: ["attention", "branding", "direction"] as const }],
};
assert.equal(marketingVideoDraftSchema.safeParse(incomplete).success, false);
assert.equal(
  marketingVideoDraftSchema.safeParse({
    ...migrated,
    clips: [{ ...migrated.clips[0]!, abcdRoles: ["branding", "connection", "direction"] }],
  }).success,
  false,
);
assert.equal(
  marketingVideoDraftSchema.safeParse({
    ...migrated,
    clips: [{ ...migrated.clips[0]!, abcdRoles: ["attention", "connection", "direction"] }],
  }).success,
  false,
);
  ...
  factualClaims: [
    {
      ...
      value: "RYC302 Clutch Kit",
      evidenceRef: "evidence-product-601",
    },
  ],
  sourceAssets: [
    {
  ...
  new Map([
    [
      "evidence-source-001",
      {
        ...
        signedGetUrl: "https://private.example/source.mp4?token=short-lived",
      },
    ],
  ]),
  "RYC302 Clutch Kit",
...
assert.throws(
  () =>
    createRemotionCompositionProps(
...
console.log(
  "PASS Remotion ABCD v3 preserves governed facts, private source boundaries, and deterministic duration",
);
