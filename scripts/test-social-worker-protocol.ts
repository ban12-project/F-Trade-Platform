...
async function main() {
  ...
  assert.deepEqual(verifySocialWorkerPayload(verified, { text: "Synthetic approved content" }), {
    text: "Synthetic approved content",
  });
  assert.throws(() => verifySocialWorkerPayload(verified, { text: "Tampered content" }), /digest/);
  await assert.rejects(
    () => verifySocialWorkerCommand(signed, command.workerId, nonceStore, now),
    /already been used/,
  );
  assert.throws(
    () =>
      signSocialWorkerCommand({
        ...
        expiresAt: new Date("2026-08-31T00:06:00.000Z"),
      }),
    /lifetime/,
  );
  ...
  await assert.rejects(
    () =>
      verifySocialWorkerCommand(
        { ...tampered, command: { ...tampered.command, payloadRef: "other-payload" } },
        command.workerId,
        nonceStore,
        now,
      ),
