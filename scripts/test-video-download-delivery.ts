...
import { approvedVideoDownloadHeaders, resolveApprovedVideoDownload } from "../lib/video/download-policy";
...
const approvedProject = videoProjectSchema.parse({
  ...
  platforms: ["facebook"],
  factualClaims: [{ field: "product.product_name", value: "Verified clutch kit", evidenceRef: "evidence-product-701" }],
  sourceAssets: [{ assetRef: "evidence-source-701", mediaType: "image", rightsEvidenceRef: "evidence-rights-701" }],
  scenes: [{ sceneId: "scene-701", prompt: "Authorized image", durationSeconds: 5, claimRefs: [], assetRefs: ["evidence-source-701"] }],
  ...
  exportArtifact: {
    ...
    measured: { container: "mp4", videoCodec: "h264", audioCodec: "aac", width: 1080, height: 1080, fps: 30, durationSeconds: 5, subtitleStreamCount: 0 },
...
const store = { async getGeneratedVideo(assetRef: string, range?: string | null) {
...
void (async () => {
  ...
  assert.equal(responseHeaders["Content-Disposition"], 'attachment; filename="f-trade-facebook-00000000.mp4"');
  assert.equal(responseHeaders["Cache-Control"], "private, no-store");
  assert.equal((await resolveApprovedVideoDownload(null, videoId, null, store, findApproved, async () => undefined)).kind, "forbidden");
