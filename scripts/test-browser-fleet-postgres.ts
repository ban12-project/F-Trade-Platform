...
async function main() {
  ...
  if (
    ...
    !url.pathname.endsWith("browser_fleet_test")
  )
    throw new Error("Only a dedicated local test database is permitted");
  ...
  try {
    ...
    await pool.query(
      await readFile(new URL("../drizzle/0028_browser_fleet.sql", import.meta.url), "utf8"),
    );
      ...
      Array.from({ length: 12 }, async () => {
        ...
        try {
          ...
          const { rows } = await client.query(
            "SELECT document FROM browser_fleet_node WHERE id=$1 FOR UPDATE",
            [nodeId],
          );
          ...
          await client.query("UPDATE browser_fleet_node SET document=$2::jsonb WHERE id=$1", [
            nodeId,
    ...
    await pool.query(insert, [
      ...
      "synthetic-hash-b",
      JSON.stringify(initialState(state.limits)),
    ]);
    await pool.query(
      "INSERT INTO browser_fleet_binding (id,node_id,account_ref,channel_ref) VALUES ($1,$2,$3,$4)",
      [randomUUID(), nodeId, "synthetic-account", "facebook"],
    );
    await assert.rejects(
      pool.query(
        "INSERT INTO browser_fleet_binding (id,node_id,account_ref,channel_ref) VALUES ($1,$2,$3,$4)",
        [randomUUID(), otherNode, "synthetic-account", "facebook"],
      ),
      (error: unknown) => (error as { code?: string }).code === "23505",
    );
    await assert.rejects(
      pool.query("UPDATE browser_fleet_node SET document=$2::jsonb WHERE id=$1", [
        nodeId,
...
main().catch(() => {
  console.error("Browser fleet PostgreSQL test failed");
  process.exitCode = 1;
});
