...
const mockFetch = async (input: URL | RequestInfo, init?: RequestInit) => {
  ...
  if (String(input).endsWith("/health"))
    return new Response(
      JSON.stringify({ ok: true, browserConnected: true, activeTabs: 0, activeSessions: 1 }),
      { status: 200 },
    );
  if (String(input).endsWith("/tabs"))
    return new Response(JSON.stringify({ tabId: "tab-001" }), { status: 200 });
  return new Response(JSON.stringify({ snapshot: "- document", refsCount: 1 }), { status: 200 });
};
const client = new CamofoxWorkerClient(
  {
    baseUrl: "http://127.0.0.1:9377",
    accessKey: "a".repeat(32),
    userId: "facebook-profile-001",
