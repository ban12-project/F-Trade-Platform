...
import {
  ...
  resolveAdminPrivateVideoPreview,
...
void (async () => {
  ...
  assert.deepEqual(await resolveAdminPrivateVideoPreview(null, "asset-video-001", store), {
    kind: "forbidden",
  });
  assert.equal(reads, 0);
  assert.deepEqual(
    await resolveAdminPrivateVideoPreview({ user: { role: "admin" } }, "not-an-asset", store),
    { kind: "not_found" },
  );
  assert.equal(reads, 0);
  const ready = await resolveAdminPrivateVideoPreview(
    { user: { role: "admin" } },
    "asset-video-001",
    store,
  );
  ...
  console.log(
    "PASS private video preview requires workspace access, a linked render, and exposes no Blob URL",
  );
})();
