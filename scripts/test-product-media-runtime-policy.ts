...
import { videoProjectSchema, type VideoProject } from "../lib/video/contracts";
import {
...
assert.deepEqual(assertCurrentProductMediaUsage(project, [record], "organic", evaluatedAt), [record]);
assert.throws(() => assertCurrentProductMediaUsage(project, [], "organic", evaluatedAt), /已不存在/);
assert.throws(() => assertCurrentProductMediaUsage(project, [{ ...record, reviewStatus: "rejected" }], "organic", evaluatedAt), /撤销|拒绝/);
assert.throws(() => assertCurrentProductMediaUsage(project, [{ ...record, rightsExpiresAt: evaluatedAt }], "organic", evaluatedAt), /已经过期/);
assert.throws(() => assertCurrentProductMediaUsage(project, [{ ...record, editingAllowed: false }], "organic", evaluatedAt), /不允许剪辑/);
assert.throws(() => assertCurrentProductMediaUsage(project, [{ ...record, publicDistributionAllowed: false }], "organic", evaluatedAt), /不允许公开发布/);
assert.throws(() => assertCurrentProductMediaUsage(project, [record], "paid_advertising", evaluatedAt), /付费广告授权/);
assert.throws(() => assertCurrentProductMediaUsage(project, [{ ...record, evidenceRef: "evidence-replaced-media-951" }], "organic", evaluatedAt), /源文件证据已经变化/);
assert.throws(() => assertCurrentProductMediaUsage(project, [{ ...record, rightsEvidenceRef: "evidence-replaced-rights-951" }], "organic", evaluatedAt), /权利证据已经变化/);
assert.throws(() => assertCurrentProductMediaUsage(project, [{ ...record, productId: "00000000-0000-4000-8000-000000000999" }], "organic", evaluatedAt), /属于其他产品/);
assert.throws(() => assertCurrentProductMediaUsage(project, [record, record], "organic", evaluatedAt), /记录重复/);
