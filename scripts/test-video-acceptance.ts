...
import { beginVideoJob, createVideoJob, failVideoJob } from "../lib/video/jobs";
...
const project = {
  ...
  factualClaims: [
    {
      ...
      value: "Synthetic clutch disc",
      evidenceRef: "evidence-product-201",
    },
  ],
  sourceAssets: [
    {
      assetRef: "asset-product-201",
      mediaType: "image" as const,
      rightsEvidenceRef: "evidence-rights-201",
    },
    {
      assetRef: "asset-subtitle-201",
  ...
  scenes: [
    {
      sceneId: "scene-201",
      prompt: "Use the approved synthetic product image with subtitles.",
      durationSeconds: 5,
      claimRefs: ["product.product_name"],
      assetRefs: ["asset-product-201", "asset-subtitle-201"],
    },
  ],
...
assert.throws(
  () =>
    videoProjectSchema.parse({
      ...project,
      scenes: [{ ...project.scenes[0], assetRefs: ["asset-unlicensed-201"] }],
    }),
  /未授权素材/,
);
assert.throws(
  () =>
    videoProjectSchema.parse({
      ...project,
      factualClaims: [{ ...project.factualClaims[0], evidenceRef: "factory-document-201" }],
    }),
  /脱敏/,
);
...
const catalog = [
  {
    ...
    verificationRef: "evidence-provider-201",
    enabled: true,
  },
];
assert.equal(
  selectVerifiedVideoModel(catalog, {
    provider: "google",
    modelId: "synthetic-video-201",
...
assert.throws(
  () =>
    selectVerifiedVideoModel([{ ...catalog[0], enabled: false }], {
      provider: "google",
      modelId: "synthetic-video-201",
      requiredCapabilities: ["text-to-video"],
      aspectRatio: "9:16",
...
const approval: ApprovalDecision = {
  id: "approval-content-201",
  aggregateId: videoId,
  gate: "gate_01_truth",
  status: "approved",
  decidedByType: "human",
  decidedById: "synthetic-reviewer",
  evidenceRef: "evidence-review-201",
};
const reviewToApproved: WorkflowEventInput = {
  eventId: "event-video-201",
  entityType: "video",
  entityId: videoId,
  fromState: "VIDEO_REVIEW_REQUIRED",
  toState: "VIDEO_APPROVED",
  actorType: "human",
  actorId: "synthetic-reviewer",
  occurredAt: "2026-08-28T01:00:00Z",
  evidenceRefs: ["evidence-review-201"],
  gate: "gate_01_truth",
  approvalRef: approval.id,
};
assert.doesNotThrow(() => assertTransition(reviewToApproved, approval));
assert.throws(
  () => assertTransition({ ...reviewToApproved, actorType: "agent" }, approval),
  /cannot perform/,
);
