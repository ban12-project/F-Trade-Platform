...
import { marketingVideoDraftSchema, marketingVideoDurationMs, type MarketingVideoDraft } from "@/lib/video/edit-contracts";
...
const abcdRoleLabels = { attention: "Attention 抓注意", branding: "Branding 露品牌", connection: "Connection 建联系", direction: "Direction 给行动" } as const;
const motionPresetLabels = { punch_in: "快速推进", hero_reveal: "产品揭示", slow_pan: "缓慢横移", cta_hold: "行动定格" } as const;
...
function ClipEditor({ draft, factOptions, onChange, disabled }: { draft: MarketingVideoDraft; factOptions: MarketingVideoEditorEntry["captionFactOptions"]; onChange: (draft: MarketingVideoDraft) => void; disabled: boolean }) {
  ...
  return <FieldSet disabled={disabled}><FieldLegend>剪辑顺序</FieldLegend><div className="flex flex-col gap-3">{draft.clips.map((clip, index) => <Card key={clip.clipId}>
    ...
    <CardContent><FieldGroup>
      <Field><FieldLabel>ABCD 节拍</FieldLabel><div className="flex flex-wrap gap-1.5">{clip.abcdRoles.map((role) => <Badge key={role} variant="outline">{abcdRoleLabels[role]}</Badge>)}</div><FieldDescription>AI 可以让同一素材承担多个节拍，但整条视频必须覆盖 ABCD。</FieldDescription></Field>
      <Field><FieldLabel htmlFor={`${clip.clipId}-motion`}>运动方式</FieldLabel><NativeSelect className="w-full" id={`${clip.clipId}-motion`} value={clip.motionPreset} onChange={(event) => patchClip(index, { motionPreset: event.target.value as keyof typeof motionPresetLabels })}>{Object.entries(motionPresetLabels).map(([value, label]) => <NativeSelectOption key={value} value={value}>{label}</NativeSelectOption>)}</NativeSelect></Field>
      {clip.sourceAnalysis ? <Field><FieldLabel>自动镜头分析</FieldLabel><div className="flex flex-wrap gap-1.5"><Badge variant="outline">区间 {(clip.sourceAnalysis.intervalStartMs / 1_000).toFixed(1)}–{(clip.sourceAnalysis.intervalEndMs / 1_000).toFixed(1)} 秒</Badge><Badge variant="outline">代表帧 {(clip.sourceAnalysis.representativeMs / 1_000).toFixed(1)} 秒</Badge><Badge variant="secondary">视觉动作 {clip.sourceAnalysis.actionScore}/100 · {{ low: "低", medium: "中", high: "高" }[clip.sourceAnalysis.actionLevel]}</Badge></div><FieldDescription>动作分只表示相邻画面的变化强度，不代表内容质量；截取范围被限制在同一检测区间内。</FieldDescription></Field> : null}
      {clip.mediaType === "video" ? <Field><FieldLabel htmlFor={`${clip.clipId}-start`}>源素材起点（秒）</FieldLabel><Input id={`${clip.clipId}-start`} type="number" min={(clip.sourceAnalysis?.intervalStartMs ?? 0) / 1_000} max={clip.sourceAnalysis ? Math.max(clip.sourceAnalysis.intervalStartMs, clip.sourceAnalysis.intervalEndMs - clip.durationMs) / 1_000 : undefined} step="0.1" value={clip.trimStartMs / 1_000} onChange={(event) => patchClip(index, { trimStartMs: Math.max(clip.sourceAnalysis?.intervalStartMs ?? 0, Math.round(Number(event.target.value) * 1_000)) })} /></Field> : null}
      <Field><FieldLabel htmlFor={`${clip.clipId}-duration`}>成片时长（秒）</FieldLabel><Input id={`${clip.clipId}-duration`} type="number" min="1" max={clip.sourceAnalysis ? Math.min(10, (clip.sourceAnalysis.intervalEndMs - clip.trimStartMs) / 1_000) : 10} step="0.1" value={clip.durationMs / 1_000} onChange={(event) => patchClip(index, { durationMs: Math.round(Number(event.target.value) * 1_000) })} /></Field>
...
function EditVideo({ projectId, entry, canReview, onDirtyChange }: { projectId: string; entry: MarketingVideoEditorEntry; canReview: boolean; onDirtyChange: (dirty: boolean) => void }) {
  ...
  return <div className="flex flex-col gap-4">
    <div className="flex flex-wrap items-center gap-2"><Badge variant="secondary">{stateLabel(entry.state)}</Badge>{processing ? <Badge>后台处理中</Badge> : null}{entry.privateTestOnly ? <Badge variant="outline">仅限私有测试</Badge> : null}<Badge variant="outline">{entry.draft.platform}</Badge><Badge variant="outline">最长 15 秒</Badge></div>
    <div><h3 className="font-medium">{entry.productName}</h3><p className="mt-1 text-sm text-muted-foreground">{entry.objective}</p></div>
    {entry.previewAssetRef ? <Card><CardHeader><CardTitle>私有预览</CardTitle><CardDescription>{entry.privateTestOnly ? "互联网测试素材不能批准、下载或发布。" : "审核通过也不会自动发布。"}</CardDescription></CardHeader><CardContent><video className="aspect-video w-full rounded-lg bg-muted" controls preload="metadata" src={`/api/video-preview/${entry.previewAssetRef}`} /></CardContent>{entry.downloadAvailable ? <CardFooter><LinkButton className="w-full" href={`/api/video-download/${entry.id}`} download prefetch={false}><DownloadIcon data-icon="inline-start" />下载 MP4</LinkButton></CardFooter> : null}</Card> : null}
    {editable ? <>
