...
import { BotIcon, CheckCircle2Icon, CopyIcon, FilePenLineIcon, PlusIcon, RotateCcwIcon, ShieldCheckIcon, XCircleIcon } from "lucide-react";
...
import { copyContentDraftToProjectAction, createContentDraftAction, decideContentReviewAction, reviseContentDraftAction } from "@/lib/actions/content";
...
import type { ContentCatalogDetail, ContentCatalogEntry, ContentCopyCandidate, ReadyProductContentSource } from "@/lib/content/store";
...
function ContentDraftForm({ projectId, products, detail }: { projectId: string; products: ReadyProductContentSource[]; detail?: ContentCatalogDetail }) {
  ...
  useEffect(() => { if (saveState.status === "success") { form.reset(revising ? form.getValues() : defaultValues(products)); router.refresh(); } }, [form, products, revising, router, saveState.status]);
...
function ContentCopy({ projectId, candidates }: { projectId: string; candidates: ContentCopyCandidate[] }) {
  const router = useRouter(); const [selected, setSelected] = useState(candidates[0]?.id ?? ""); const [state, action, pending] = useActionState(copyContentDraftToProjectAction, initialContentActionState);
  useEffect(() => { if (state.status === "success") router.refresh(); }, [router, state.status]);
  function copy() { if (!selected) return; const data = new FormData(); data.set("projectId", projectId); data.set("sourceContentId", selected); startTransition(() => action(data)); }
  return <Card><CardHeader><CardTitle>从其他项目复制</CardTitle><CardDescription>创建独立待审草稿；后续修订和审核不会影响源项目。</CardDescription></CardHeader><CardContent>{candidates.length ? <Field><FieldLabel>源内容</FieldLabel><Select value={selected} onValueChange={(value) => value && setSelected(value)}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{candidates.map((item) => <SelectItem key={item.id} value={item.id}>{item.projectTitle} · {item.productName} · {item.hook}</SelectItem>)}</SelectGroup></SelectContent></Select></Field> : <p className="text-sm text-muted-foreground">其他项目暂无可复制内容。</p>}</CardContent><CardFooter className="flex-col items-stretch gap-3"><Button disabled={pending || !selected} onClick={copy}><CopyIcon data-icon="inline-start" />复制为新草稿</Button>{state.message ? <p className={state.status === "error" ? "text-sm text-destructive" : "text-sm text-muted-foreground"}>{state.message}</p> : null}</CardFooter></Card>;
}

export function ContentPanel({ projectId, products, entries, copyCandidates, detail, canReview }: { projectId: string; products: ReadyProductContentSource[]; entries: ContentCatalogEntry[]; copyCandidates: ContentCopyCandidate[]; detail: ContentCatalogDetail | null; canReview: boolean }) {
