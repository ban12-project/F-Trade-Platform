...
export function ProjectWorkspace({ project, projects, tasks, settingsPanel, membersPanel, stages, activeStage, panel, basePath = `/workspace/${project.id}` }: { project: WorkspaceProjectDetail; projects: WorkspaceProjectSummary[]; tasks: WorkspaceTaskSummary[]; settingsPanel?: ReactNode; membersPanel?: ReactNode; stages: ProjectStage[]; activeStage: string; panel: ReactNode; basePath?: string }) {
  ...
  return <WorkspaceDirtyProvider><main id="main-content" className="min-h-screen bg-muted/30 pb-24">
    <header className="sticky top-0 z-20 border-b bg-background/90 backdrop-blur-xl"><div className="mx-auto flex max-w-[96rem] flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6"><div className="flex min-w-0 items-center gap-3"><LinkButton href="/workspace" size="icon" variant="ghost" aria-label="返回工作台"><ArrowLeftIcon /></LinkButton><div className="min-w-0"><h1 className="truncate text-lg font-semibold tracking-tight">{project.title}</h1><div className="mt-1 flex flex-wrap gap-2"><Badge variant="secondary">{project.kind === "marketing" ? "营销项目" : "销售项目"}</Badge><Badge variant="outline">{project.status === "active" ? "进行中" : "已归档"}</Badge><Badge variant="outline">关键动作需人工确认</Badge></div></div></div>{membersPanel ? <div className="flex items-center gap-2">{membersPanel}</div> : null}</div></header>
    <div className="mx-auto max-w-[96rem] px-4 py-5 sm:px-6">
      <nav aria-label="项目阶段" className="overflow-x-auto pb-2"><ol className="flex min-w-max items-stretch gap-1">{stages.map((item, index) => <li key={item.id} className="flex items-center"><Link href={`${basePath}?panel=${item.id}`} aria-current={item.id === activeStage ? "step" : undefined} className={`flex min-h-14 w-40 flex-col justify-center rounded-xl border px-3 outline-none transition-[background-color,border-color,transform] duration-[120ms] active:scale-[0.98] focus-visible:ring-3 focus-visible:ring-ring/50 ${item.id === activeStage ? "border-primary bg-primary text-primary-foreground" : "bg-background hover:bg-muted"}`}><span className="text-xs opacity-75">步骤 {index + 1}</span><span className="text-sm font-medium">{item.label}</span></Link>{index < stages.length - 1 ? <ArrowRightIcon className="mx-1 size-4 text-muted-foreground" aria-hidden="true" /> : null}</li>)}</ol></nav>
      <section aria-label="当前阶段" className="mt-4 grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(24rem,34rem)]">
        <div className="min-w-0 space-y-5"><Card><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardDescription>当前步骤</CardDescription><CardTitle role="heading" aria-level={2} className="mt-1">{stage.label}</CardTitle><p className="mt-2 text-sm text-muted-foreground">{stage.description}</p></div><Badge>{stageTasks.length ? `${stageTasks.length} 项待处理` : "当前无待办"}</Badge></div></CardHeader></Card><Card><CardHeader><CardTitle role="heading" aria-level={2}>待处理事项</CardTitle><CardDescription>先处理这里的下一动作；也可以在右侧新建或查看本步骤的业务记录。</CardDescription></CardHeader><CardContent>{stageTasks.length ? <div className="divide-y">{stageTasks.map((task) => <Link key={`${task.taskType}-${task.id}`} href={`${basePath}?panel=${task.nodeKind}&item=${task.id}`} className="group flex min-h-16 items-center gap-3 py-3 outline-none focus-visible:ring-3 focus-visible:ring-ring/50"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{task.title}</p><p className="mt-1 truncate text-xs text-muted-foreground">{task.detail}</p></div><Badge variant={task.priority === "review" ? "default" : "secondary"}>{task.actionLabel ?? "打开"}</Badge><ArrowRightIcon className="size-4 text-muted-foreground transition-transform duration-[120ms] group-hover:translate-x-0.5" /></Link>)}</div> : <p className="py-8 text-center text-sm text-muted-foreground">本步骤暂无待处理事项，可在右侧创建或查看业务记录。</p>}</CardContent></Card></div>
