...
function toNodes(document: WorkspaceCanvasDocument): CanvasNode[] { return document.nodes.map((node) => ({ id: node.id, position: node.position, data: { label: node.kind === "video" ? "营销视频" : node.label, kind: node.kind, aggregateId: node.aggregateId }, draggable: !node.locked, deletable: false, style: { borderColor: nodeTone[node.kind] ?? "#64748b" } })); }
...
export type ProjectCanvasVideoData = { products: ReadyVideoProductSource[]; entries: MarketingVideoEditorEntry[]; canReview: boolean };

function Inspector({ project, selected, onClose, videoEditor, onDirtyChange }: { project: WorkspaceProjectDetail; selected: CanvasNode | null; onClose: () => void; videoEditor?: ProjectCanvasVideoData; onDirtyChange: (dirty: boolean) => void }) {
...
export function ProjectCanvas({ project, videoEditor }: { project: WorkspaceProjectDetail; videoEditor?: ProjectCanvasVideoData }) {
  ...
  function setPanel(kind: string | null) { if (kind === panel) return; if (panelDirty && !window.confirm("剪辑稿还有未保存修改，确定离开吗？")) return; const params = new URLSearchParams(searchParams.toString()); if (kind) params.set("panel", kind); else params.delete("panel"); router.replace(params.size ? `${pathname}?${params}` : pathname, { scroll: false }); setPanelDirty(false); }
  ...
  return <main ref={focusRef} tabIndex={-1} id="main-content" className="fixed inset-0 overflow-hidden bg-muted outline-none" aria-label={`${project.title} 项目画布`}><ReactFlow className="bg-background" nodes={nodes} edges={document.edges.map((edge) => ({ id: edge.id, source: edge.source, target: edge.target }))} onNodesChange={onNodesChange} onNodeClick={(_, node) => setPanel(String(node.data.kind))} onPaneClick={() => setPanel(null)} fitView nodesDraggable><Background /><Controls position="top-right" /><MiniMap className="hidden md:block" nodeColor={(node) => nodeTone[String(node.data?.kind)] ?? "#64748b"} position="bottom-left" /></ReactFlow>
    ...
    <Inspector project={project} selected={selected} onClose={() => setPanel(null)} videoEditor={videoEditor} onDirtyChange={setPanelDirty} />
  </main>;
}
