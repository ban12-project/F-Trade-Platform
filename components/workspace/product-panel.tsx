...
function ProductAgentForm({ projectId, configured }: { projectId: string; configured: boolean }) {
  ...
  const form = useForm<AgentValues>({ resolver: zodResolver(productAgentRunFormSchema), defaultValues: { sourceRef: "", evidenceRef: "", sourceText: "", hasUpload: false } });
  useWorkspaceDirty("product-agent-import", form.formState.isDirty);
  useEffect(() => { if (state.status === "success") { form.reset(); if (fileRef.current) fileRef.current.value = ""; router.refresh(); } }, [form, router, state.status]);
  function submit(values: AgentValues) { const data = new FormData(); data.set("projectId", projectId); data.set("sourceRef", values.sourceRef ?? ""); data.set("evidenceRef", values.evidenceRef ?? ""); data.set("sourceText", values.sourceText); const file = fileRef.current?.files?.[0]; if (file) data.set("document", file); startTransition(() => action(data)); }
  return <Card><CardHeader><CardTitle>从获授权资料导入</CardTitle><CardDescription>上传 PDF、CSV、XLS 或 XLSX，或粘贴已预处理文本；结果仍需 Gate 01。</CardDescription></CardHeader><CardContent><form id="product-agent-import" onSubmit={form.handleSubmit(submit)}><FieldGroup><Field><FieldLabel htmlFor="agent-document">产品资料</FieldLabel><Input ref={fileRef} id="agent-document" type="file" accept=".pdf,.csv,.xls,.xlsx" onChange={(event) => form.setValue("hasUpload", Boolean(event.target.files?.length), { shouldValidate: true })} /><FieldDescription>原始文件只进入 Private Blob，最大 25MB。</FieldDescription></Field><Field data-invalid={!!form.formState.errors.sourceRef}><FieldLabel htmlFor="agent-source">来源引用</FieldLabel><Input id="agent-source" placeholder="source-catalog-001" {...form.register("sourceRef")} /><FieldError errors={[form.formState.errors.sourceRef]} /></Field><Field data-invalid={!!form.formState.errors.evidenceRef}><FieldLabel htmlFor="agent-evidence">字段证据引用</FieldLabel><Input id="agent-evidence" placeholder="evidence-catalog-001" {...form.register("evidenceRef")} /><FieldError errors={[form.formState.errors.evidenceRef]} /></Field><Field data-invalid={!!form.formState.errors.sourceText}><FieldLabel htmlFor="agent-text">已授权资料文本</FieldLabel><Textarea id="agent-text" rows={8} {...form.register("sourceText")} /><FieldError errors={[form.formState.errors.sourceText]} /></Field></FieldGroup></form></CardContent><CardFooter className="flex-col items-stretch gap-3"><Button form="product-agent-import" type="submit" disabled={pending || !configured}>{pending ? <Spinner data-icon="inline-start" /> : <BotIcon data-icon="inline-start" />}生成待审核草稿</Button>{!configured ? <p className="text-sm text-muted-foreground">管理员需先在工作区设置中配置 Product Agent。</p> : null}{state.message ? <p className={state.status === "error" ? "text-sm text-destructive" : "text-sm text-muted-foreground"} aria-live="polite">{state.message}</p> : null}</CardFooter></Card>;
}
...
export function ProductPanel({ projectId, entries, detail, canReview, agentConfigured }: { projectId: string; entries: ProductCatalogEntry[]; detail: ProductCatalogDetail | null; canReview: boolean; agentConfigured: boolean }) {
