...
let installationId;
try { installationId = (await readFile(`${stateDir}/installation-id`, "utf8")).trim(); }
catch (error) {
...
if (!Number.isInteger(maxSlots) || !Number.isFinite(reserveMb)) throw new Error("node_capacity_invalid");
const docker = dockerClient(process.env.DOCKER_SOCKET ?? "/var/run/docker.sock");
const image = await docker("GET", `/images/${encodeURIComponent(process.env.BROWSER_IMAGE ?? "ftrade-browser:lease-v1")}/json`);
if (image.Config?.Labels?.["io.ftrade.lease-watchdog"] !== "1") throw new Error("reviewed_watchdog_image_required");
...
if (process.env.BROWSER_TASK_ADAPTER) {
  ...
  if (typeof extra.execute !== "function" || !Array.isArray(extra.capabilities) || extra.capabilities.some((c) => !["inbox", "publish"].includes(c))) throw new Error("reviewed_adapter_invalid");
  adapter = { capabilities: ["interactive", ...extra.capabilities], execute: extra.execute };
}
const slots = new Map();
let gatewayOrigin = ""; let stopping = false; let pendingClaim = null;
async function nodeCall(operation, fields = {}) {
  ...
  if (!response.ok) {
    await response.body?.cancel(); const error = new Error(`platform_http_${response.status}`);
...
async function checkpoint() {
  const safe = [...slots.values()].map((s) => ({ id: s.run.id, leaseId: s.run.leaseId, accountId: s.run.accountId, kind: s.run.kind, stopped: !!s.stopped }));
...
async function browserRequest(slot, path, body, timeout = 30_000) {
  ...
  const response = await fetch(`http://127.0.0.1:${slot.apiPort}${path}`, {
    ...
    headers: { Authorization: `Bearer ${slot.accessKey}`, ...(body ? { "Content-Type": "application/json" } : {}) },
    ...(body ? { body: JSON.stringify(body) } : {}),
    signal: AbortSignal.any([slot.abort.signal, AbortSignal.timeout(timeout)]),
  });
  if (!response.ok) { await response.body?.cancel(); throw new Error("browser_request_failed"); }
  return response;
}
...
await nodeCall("recover", { stoppedRunIds: sync.runs.filter(isLive).map((r) => r.id), capabilities: adapter.capabilities });
const gateway = createGateway({ appOrigin, nodeCall, slots, port: Number(process.env.GATEWAY_PORT ?? 9400) });
console.log("browser_node_ready");

async function stop(slot, outcome = "failed") {
  if (slot.stopPromise) return slot.stopPromise;
  slot.stopping = true; gateway.closeRun(slot.run.id);
  slot.stopPromise = (async () => {
    if (!slot.stopped) {
      ...
      if (slot.apiPort && slot.expiresAt > Date.now()) {
        try {
          const response = await fetch(`http://127.0.0.1:${slot.apiPort}/sessions/${slot.run.accountId}/storage_state`, {
            headers: { Authorization: `Bearer ${slot.accessKey}` }, redirect: "error", signal: AbortSignal.timeout(5000),
          });
          await response.body?.cancel();
        } catch { /* Graceful server shutdown still attempts persistence. */ }
      }
      await stopContainer(docker, slot.containerId ?? slot.name);
      try { await docker("DELETE", `/networks/${slot.name}`); } catch (error) { if (error.status !== 404) throw error; }
      slot.stopped = true; await checkpoint();
    }
    await nodeCall("finish", { runId: slot.run.id, leaseId: slot.run.leaseId, stopped: true, outcome });
    slots.delete(slot.run.id); await checkpoint();
  })();
  try { await slot.stopPromise; }
  finally { slot.stopPromise = null; }
}
async function launch(slot) {
  const spec = slot.spec;
  await docker("POST", "/volumes/create", { Name: spec.volume, Labels: { "io.ftrade.node": nodeId, "io.ftrade.account": slot.run.accountId } });
  await docker("POST", "/networks/create", { Name: spec.name, Driver: "bridge", Labels: spec.labels });
  if (slot.stopping) return;
  const created = await docker("POST", `/containers/create?name=${spec.name}`, spec.body); slot.containerId = created.Id;
  if (slot.stopping) return;
  ...
  for (let i = 0; i < 60; i++) {
    ...
    try { const response = await browserRequest(slot, "/health", undefined, 2000); await response.body?.cancel(); break; }
    catch { if (i === 59) throw new Error("browser_start_timeout"); await sleep(1000, undefined, { signal: slot.abort.signal }); }
  }
  ...
  const opened = await browserRequest(slot, "/tabs", { userId: slot.run.accountId, sessionKey: slot.run.id, url: "https://api.ipify.org/?format=json", trace: false }, 120_000);
  await opened.body?.cancel();
  if (slot.stopping) return;
  if (slot.run.kind === "interactive") {
    const login = await browserRequest(slot, "/tabs", { userId: slot.run.accountId, sessionKey: slot.run.id, url: "https://www.facebook.com/", trace: false }, 120_000);
    await login.body?.cancel();
  }
  for (let i = 0; i < 30; i++) {
    const response = await browserRequest(slot, "/vnc/status"); const status = await response.json();
    if (status.running === true) break;
    if (i === 29) throw new Error("vnc_not_ready"); await sleep(1000, undefined, { signal: slot.abort.signal });
  }
  const renewed = await nodeCall("heartbeat", { runId: slot.run.id, leaseId: slot.run.leaseId, ready: true });
  ...
  slot.ready = true; slot.readyAt = Date.now();
  // Secrets already live in this isolated runtime. Drop the claim's copy.
  delete slot.run.proxy; delete slot.spec;
}
async function heartbeat(slot) {
  if (slot.stopping) { await stop(slot, slot.outcome ?? "failed"); return; }
  if (slot.expiresAt <= Date.now()) { slot.outcome = "unknown"; await stop(slot, "unknown"); return; }
  const renewed = await nodeCall("heartbeat", { runId: slot.run.id, leaseId: slot.run.leaseId, ready: !!slot.ready });
  if (!renewed.active) { await stop(slot, slot.run.kind === "publish" ? "unknown" : "failed"); return; }
  slot.expiresAt = localDeadline(renewed, renewed.leaseUntil);
  const pendingConnection = Number(renewed.connectBefore ?? 0) > renewed.serverNow;
  if (slot.ready && slot.run.kind === "interactive" && (
    (!slot.connected && !pendingConnection && Date.now() - slot.readyAt > 60_000) ||
    (slot.disconnectedAt && Date.now() - slot.disconnectedAt > 15_000))) {
    await stop(slot, "completed"); return;
  }
...
async function tick() {
  ...
  try {
    await Promise.all([...slots.values()].map(async (slot) => {
      try { await heartbeat(slot); }
      catch { if (slot.expiresAt <= Date.now()) await stop(slot, "unknown").catch(() => {}); }
    }));
    if (slots.size >= maxSlots || [...slots.values()].some((s) => s.stopping)) return;
    pendingClaim ??= randomUUID();
    const result = await nodeCall("claim", { requestId: pendingClaim, availableMemoryMb: Math.max(0, Math.floor(freemem() / 1024 / 1024 - reserveMb)), localSlots: maxSlots });
    pendingClaim = null;
    ...
    const slot = { run: result.run, spec, name: spec.name, accessKey: spec.accessKey, vncPassword: spec.vncPassword,
      expiresAt, gatewayOrigin, abort: new AbortController(), ready: false, stopping: false };
    slots.set(result.run.id, slot); await checkpoint();
    slot.startPromise = launch(slot);
    void slot.startPromise.then(async () => {
      if (!slot.ready || slot.stopping || slot.run.kind === "interactive") return;
      try {
        const outcome = await adapter.execute({ run: { ...slot.run }, signal: slot.abort.signal,
          browserRequest: (path, body) => browserRequest(slot, path, body) });
        slot.outcome = ["completed", "needs_login", "needs_2fa", "checkpoint", "failed", "unknown"].includes(outcome) ? outcome : "unknown";
      } catch { slot.outcome = "unknown"; }
      await stop(slot, slot.outcome);
    }).catch(() => { slot.outcome = "failed"; void stop(slot, "failed").catch(() => {}); });
  } catch { console.error("browser_node_poll_failed"); }
  finally { ticking = false; }
}
const timer = setInterval(() => { void tick(); }, 10_000);
await tick();
async function shutdown() {
  if (stopping) return; stopping = true; clearInterval(timer); gateway.close();
  await Promise.allSettled([...slots.values()].map((slot) => stop(slot, slot.run.kind === "publish" ? "unknown" : "failed")));
  process.exit(0);
}
