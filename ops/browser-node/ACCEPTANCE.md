...
2. 从已停止的 Sandbox 开始，先执行一个只读会话检查：出口必须与审核 IP 一致，Facebook 当前身份与账号绑定一致，Messenger 必须到达 `ready` 且恢复完成。若出现明确的密码／TOTP／PIN 恢复页，再走经复核的 version 2 恢复契约；当前生产 `observe-only` 配置会拒绝因素领取，不能假装已验收自动恢复。CAPTCHA 或设备批准交给人完成原运行，失败或不确定时停止，不重新提交已执行因素。
