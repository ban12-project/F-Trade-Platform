...
2. 先在 Podman 和合成页面验证视频编辑器与回执逻辑，再在同一 Sandbox 的隔离诊断副本中复核真实 Facebook 视频编辑器的身份、受众、编辑器内唯一文件输入和**发布后成功提示中可见的唯一 Reel 链接**。上传后 Facebook 可能替换编辑器并重置受众，须重新核对身份、恢复 `Only me`，再验证正文。视频编辑器不一定显示文件名；以唯一可见 blob 视频的 SHA-256 与已审核 MP4 的摘要一致作为附件证据，授权后和点击前各核对一次。视频页面契约必须配置经审核的 `videoReceipt` 容器、链接和完整成功文案，点击前该提示不存在，点击后链接必须属于 Facebook 且为规范 Reel 路径。若真实页面没有这种同一次提交的关联信号，应保持 `textOnly: true`，另行设计可证实的回执契约，不能仅凭个人主页出现一个新 Reel 归因于当前任务。只有扩大私有 `publication.json` 的视频审核范围后才移除 `textOnly: true`；先用合成页面测试 `pnpm exec playwright test tests/e2e/facebook-driver.spec.ts` 与 `pnpm test:video-social-publication` 核对拒绝和回执路径。隔离副本的无点击预检不算真实投递。
...
先前一条私有视频已由账号持有人核对实际 Reel 与 `Only me` 受众，并在应用中作为**人工核对**结案；原始自动回执仍是 `unknown`。这不算视频自动回执通过。当前没有独立 DM 发送账号，因此真实入站、去重及 RFQ 关联也仍未验收。每项生产验收都记录任务／发布 ID、审核引用、镜像修订、起止状态、可观察结果及失败阶段；只记录不含私有正文和凭据的最小证据。视频与 DM 分别通过后才能标记为已验收。
