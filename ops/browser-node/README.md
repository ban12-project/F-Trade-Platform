...
每个请求绑定 UUID 和载荷摘要，消息与回执同事务写入。相同请求重放返回原计数；相同编号改稿拒绝。每运行最多 100 批，每批 1–20 条；HTTP 总请求最多 256 KiB，Agent 在 250000 字节处提前拒绝过大封包。正文超过单批容量时由采集器拆批，不能截断成另一条消息。运行终止后旧请求拒绝，内部摘要回执不进入管理 UI；消息正文不写入节点文档或审计。

数据库回归直接执行实际 Route Handler 和 broker，覆盖签名、HTTP 鉴权/大小限制、租约、过期与重放。尚未通过真实 HTTP 网络、VPS 或 Facebook 页面采集验收；内置适配器仍仅支持显式配置的发布，默认 Agent 仍只有人工交互。下一步是页面收件箱采集和轮询完成证据，不能通过空实现返回 completed 来宣称完成检查。
