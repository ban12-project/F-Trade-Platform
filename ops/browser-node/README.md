...
固定上游通用 `/tabs/:id/type` 会将输入全文交给 `tab:type` 插件事件，其异常还可能包含输入值；因此保存密码不能直接通过该接口执行。新增 `login-plugin/` 作为浏览器镜像内的独立插件，配置 `ftrade-login.enabled` 默认为 false，当前 Agent 不请求或发送登录密码。平台所有者会话的一次性授权、节点凭据领取和人工入口仍需后续接通；本阶段不能宣称已保存密码登录功能可用。
