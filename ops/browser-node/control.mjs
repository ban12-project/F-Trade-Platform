...
async function request(slot, path, body) {
  if (!Number.isInteger(slot.apiPort) || slot.apiPort < 1 || slot.apiPort > 65535 ||
      typeof slot.accessKey !== 'string' || !slot.accessKey) throw new Error('control_unavailable');
  ...
  if (!response.ok) { await response.body?.cancel(); throw Object.assign(new Error('control_request_failed'), { status: response.status }); }
...
export async function acquireBrowserControl(slot) {
  ...
  if (ttlMs < 1000 || typeof slot.run.accountId !== 'string' || !slot.run.accountId) throw new Error('control_unavailable');
  const state = {}; leases.set(slot, state); slot.controlPaused = true;
  const started = Date.now();
  let grant;
  try {
    grant = await request(slot, '/control/acquire', { userId: slot.run.accountId, ttlMs });
    if (grant.owner !== slot.run.accountId || !/^[a-f0-9]{64}$/.test(grant.capability) ||
        !Number.isSafeInteger(grant.expiresAt)) throw new Error('control_response_invalid');
  } catch (error) {
  ...
  return {
    ...
    release() {
      ...
      if (!releasePromise) releasePromise = (async () => {
        try {
          const result = await request(slot, '/control/release', { userId: slot.run.accountId, capability: grant.capability });
          ...
          if (status.enabled !== true || status.running !== false || status.watcherRunning !== false) throw error;
        }
        leases.delete(slot); slot.controlPaused = false; slot.controlFailure = false;
      })().catch(error => { releasePromise = null; slot.controlFailure = true; throw error; });
