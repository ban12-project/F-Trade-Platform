...
test("synthetic browser acceptance reaches opportunity through every human gate", async ({ page }) => {
  ...
  for (const state of ["PRODUCT_READY", "CONTENT_PUBLISHED", "VIDEO_APPROVED", "RFQ_READY", "QUOTE_SENT", "DELIVERY_CONFIRMATION_CONFIRMED"] as const) await expect(page.getByText(state, { exact: true })).toBeVisible();
