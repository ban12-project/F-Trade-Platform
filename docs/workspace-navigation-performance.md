...
新的 `WorkspaceShell` 在 layout 中保持挂载；导航数据单独通过 Suspense 加载，页面 children 只渲染一次。底部 dock 已移除，桌面侧栏与窄屏抽屉共享同一套栏目。新工作窗口和页面继续共用 dirty provider。

首页只保留一个可筛选的任务清单；产品、内容/发布、客户/询盘列表从同一个按成员限定的快照提取最小展示字段。具体对象和新建上下文使用稳定的项目内路由，历史查询参数入口仍兼容。引用产品可以从销售项目查看，写入控件保持只读。上传证据后也刷新新对象路由。

新增对象列表与详情入口保持 `prefetch = "partial"`。浏览器回归覆盖静态标题/返回控件、390px 首屏入口、持久导航、未保存保护、旧链接、错误对象、授权范围及真实数据库中的完整合成销售流程。这些检查验证交互和技术契约，不替代真实用户的可用性试用。
