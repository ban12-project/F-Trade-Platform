# 按需 Browser Sandbox 迁移（#332）

用户于 2026-09-08 明确要求：Browser Sandbox 仅在需要时启动，不常驻。
本迁移延续 #322 / PR #323 的节点、账号与任务授权约束，最终验收暂缓。

## 生命周期契约

- 开启计算的触发仅为人工接管请求或已获授权且实际到期的业务任务。节点列表刷新、状态查询、保活与空队列轮询不得启动或恢复 Sandbox。
- 平台负责按需调度；不能把当前常驻 Agent 的空轮询直接搬进长期运行的 Sandbox。
- 任务完成、用户关闭接管、无用户连接超时或授权撤销后，停止浏览器，再停止整个 Sandbox。运行超时是兜底，不是正常回收机制。
- 没有任务时不得延长会话。并发启动、停止和恢复必须以数据库中的操作标识和状态串行协调；模糊结果先查已有实例，不能创建替代实例并丢弃旧状态。
- 账号独立的 profile 必须在停止前一致落盘；恢复要重启 Docker 和 Agent，旧租约按现有恢复协议终止或确认，不恢复未知发布尝试。
- 列表状态只能使用不恢复会话的 SDK 元数据读取。不能调用可能自动恢复的 runCommand/fs 来探测一个已停止实例。
- 保留固定代理和出口校验、一次性接管票据及 WebSocket 到期撤销。没有账号授权或有效平台节点 Key 时，不启动真实账号运行。
- 停止计算不等于零费用：持久化快照可能继续收取存储费用，保留与删除策略需明确。

## 当前验证范围

已在真实 Vercel Sandbox 的 Ubuntu 环境安装并启动 Docker，并成功执行无网络的 hello-world 容器；实例已删除。这证明 Docker 执行基础，不证明完整 worker 或真实账号可用。

`scripts/test-browser-sandbox-runtime.mts` 是显式执行的实时验证，不接入普通 CI 自动计费。它只上传已提交的运行代码和固定上游源码，构建镜像，运行现有合成镜像 smoke，再停止／恢复 Sandbox 校验合成卷标记，最后删除实例和测试快照。它不包含真实账号、代理密码或客户资料。

首轮验证暴露的兼容性差异：当前 SDK 按 name 标识持久实体；快照有效期最小一天；默认工作目录不能硬编码为旧 runtime 的目录。脚本使用已核实的 name、最小有效期和明确的归档路径。失败轮次不记为通过。

## 待实现与验证

平台管理操作和持久化调度状态、按需唤醒与空闲回收、实际 HTTPS/noVNC 接管、并发/撤销恢复回归、账号 profile 一致性和部署接入均未完成。镜像或合成卷验证通过不能代替上述结果，也不能关闭 #332。

## 真实基础验证结果（2026-09-08）

使用 PR 323 的 `e85756cca77d7573e652c5b22e6b68e3275f690e` 与固定上游 `e5a36f5cd0332fde6597de474329a308a53a0716`：

- Sandbox 内安装 Docker/Compose，通过 Agent、Camofox 基础层与浏览器镜像的实际构建。
- 原 `test-browser-images.sh` 全部通过，包括 native Node 24、浏览器/Node 22 依赖、Camofox health、独立看门狗终止与 Compose 配置。未向真实 Facebook 发请求。
- 创建只含 SYNTHETIC_PROFILE 标记的 Docker named volume，正常停止 Docker 和 Sandbox，恢复同一 Sandbox 并重启 Docker后，原镜像可用且标记逐字一致。
- 测试结束成功停止并删除 Sandbox 及测试快照，没有留下常驻计算。首轮失败修正后才取得上述结果。

这不是已登录浏览器的 cookie/加密历史恢复验证，不证明跨重启发布回执、固定代理或 noVNC 外网接管。后续平台生命周期实现仍须满足上方按需契约。

## 按需退出与整体停止（实现基础）

Agent 新增部署级 `BROWSER_NODE_ON_DEMAND=1`。连续成功领取到空队列且本地无运行任务时，默认 30 秒后关闭网关并退出。收到任务或发生领取错误会重置空闲计时；旧 VPS 默认模式不因空队列退出。按需部署必须使用不自动重启的 Agent 容器，不能套用 VPS 的 `unless-stopped` 重启策略。

`stopIdleBrowserSandboxSession` 接收固定的 SDK Session，而非会自动恢复的 Sandbox facade。它核对 sessionId 和 Agent 的节点标签，活动 Agent 不停止；已退出 Agent 会先清理该节点的容器、停止 Docker，再停止整个会话。清理失败也尝试停止计算，但返回未确认状态，不能据此清除旧租约或记为正常释放。旧会话回调不会停止新会话。

验证：5 项空闲策略/实际 Agent 子进程回归、6 项 session 停止/身份/失败分支回归及既有 65 项节点回归通过。实际 Agent 使用本机 HTTPS broker 和 Docker 协议夹具；没有真实账号。实时 `test-browser-sandbox-idle.mts` 在 Vercel 内安装 Docker，构造退出的无网络合成 Agent 容器，再调用真实回收函数；提供方确认 VM 已停止，重复检查保持相同 sessionId 且未恢复。测试实例已删除。类型与格式检查通过。

上述函数尚未接入平台调度，不能宣称生产 worker 已自动按需运行。仍需持久化操作记录、授权校验、唤醒入口与 HTTPS 接管集成。

## 持久化启停协调（2026-09-10）

`0034_browser_sandbox.sql` 与 `sandbox-lifecycle.ts` 新增数据库协调层。登记仅创建元数据；空队列或已撤销节点不启动。启动和停止先锁定同一 broker 节点，再写入唯一操作 ID。只有赢得操作的调用方可以在事务提交后派发云端操作。请求失败或超时保留原操作并进入 unknown，不能据此另建实例；恢复必须查询原实例后确认结果。

回收以已退出 Agent 的确切 sessionId 为前提。新任务即使在停止期间进入，也保留在原队列，等旧会话停止后再启动；旧操作的迟到结果不能覆盖新的操作。这些辅助函数只供服务端事务使用，调用方仍须校验用户／任务授权，不能直接作为 HTTP 或 Action 入口。

真实 PostgreSQL 回归覆盖 12 个并发启动及停止请求、空队列、旧回调、未知结果恢复、停止期间新增账号任务和撤销后拒绝启动。类型和迁移检查通过。这里没有派发 Workflow、调用 Vercel SDK 或保存节点密钥；生产接入仍未完成。

## 托管节点登记与密钥（2026-09-10）

管理命令新增 `create-sandbox`，经现有 Server Action 的身份检查和事务内会话／权限检查。当前入口要求显式 `BROWSER_SANDBOX_ENABLED=1`，自动派发完成前不要在生产启用。登记只分配一个浏览器的默认容量并创建停止状态记录，未提供公网网关时保存 null，不编造接管域名。

节点专用密钥以 AES-GCM 加密保存，使用现有部署密钥环，但以独立 `node-access` 用途和节点 ID 绑定，不能挪作账号密码或复制到其他节点。创建、查询和轮换均不向页面返回托管节点密钥；撤销清空密文并使旧密钥失效。创建、列表查询、轮换和撤销均未调用云端 SDK。真实数据库回归已验证上述授权、隔离和不启动行为；既有浏览器发布、登录及收件回归通过。

尚需实现云端运行配置、派发／恢复调度、网关地址回填、按需管理 UI 和 HTTPS 接管。这个入口不是已完成的生产部署。

## Sandbox 内启动配置（2026-09-10）

`compose.sandbox.yaml` 定义单个 Agent，明确 `restart: no`、禁止拉取镜像、空闲 30 秒退出，节点密钥使用只读文件挂载。Agent 带节点及操作标签，供平台识别对应会话。恢复逻辑因此改为只停止同时具有节点和任务标签的浏览器容器，避免把 Agent 自身当成遗留浏览器停止。

`start-sandbox.sh` 只在已授权且正在运行的 VM 内执行。它检查 root 所有的 0600 配置／密钥文件、节点及操作标识和不可变本地镜像 ID，通过文件锁串行启动。相同操作对应的 Agent 已退出时，不再次启动；不同操作遇到仍运行的 Agent 时要求协调，不能强制替换。脚本不删除 profile 卷。平台生成运行配置和实际调用该脚本的部分仍待接入。

实际 Agent 子进程的恢复过滤与空闲退出回归、会话停止回归共 11 项通过，既有 65 项节点回归通过；启动脚本语法通过检查。新增 Compose 配置验证随双架构镜像 CI 执行。以上尚不证明新启动脚本已在真实 Sandbox 中完成端到端运行。

## 实测发现的资源控制兼容问题（2026-09-10）

首次实际调用启动脚本失败：未初始化嵌套 cgroup 时，Docker 创建的层级进入 threaded 模式，带内存上限的 Agent 无法启动。无资源限制的 hello-world 通过不能证明此配置可用。失败实例已删除。

`prepare-sandbox-cgroups.sh` 采用 [Moby 的嵌套 cgroup 初始化方法](https://github.com/moby/moby/blob/master/hack/dind)，在专用 VM 的首个 Docker 容器之前把现有进程放入子 domain，并启用内存及线程控制器；没有移除容器限制，也不操作 VM 命名空间之外的层级。已经进入不兼容模式时拒绝继续。重试有次数上限，不能永久阻塞启动。

独立真实 Sandbox 探针已验证修复前限内存容器启动失败、初始化后相同配置成功，容器内核 `memory.max` 实际为 67108864（64 MiB）。探针实例均已删除。恢复时 cgroup 是新内核状态，必须在重启 Docker 前重新初始化，不能把文件系统快照当成已恢复资源控制配置。

`test-browser-sandbox-start.mts` 使用已提交 Agent 源码、空队列 HTTPS 合成 broker、临时节点 Key 和不可变本地测试镜像，实际执行启动脚本。它测试重复启动、Agent 空闲退出、HTTPS 接管静态资源及整机回收，不发放真实账号任务，不证明浏览器 profile、代理或 WebSocket 接管。

使用 `8b9bed1` 的完整合成实测通过：Agent 正常启动，容器内核内存上限为 536870912（512 MiB），重复调用返回 already-running，公开 HTTPS 地址可以访问 `/viewer`；空闲退出后同一操作返回 already-exited，没有重新启动。随后真实 session 回收函数停止整个 VM，元数据确认 stopped 且 sessionId 未变。最后删除实例和测试快照，清理完成后才写入成功结果。生产派发与实际业务接管仍未完成。

## 私密配置下发与网关绑定

`startBrowserSandboxRuntime` 对固定的运行中 Session 下发私密配置。密钥只进入 0600 文件，不出现在命令参数；过期会话不执行 I/O。启动和部分上传失败均尝试清理临时文件，清理未确认时不能返回成功。脚本在文件锁内检查已存在 Agent，使用校验后的 Compose 配置快照启动，避免校验和执行读取不同配置。

`3413027` 的真实 Sandbox 测试已通过，成功结果文件在清理完成后写入：私密下发、重复操作、临时文件清理、512 MiB 内存上限、HTTPS 静态页面、空闲退出及整机回收均通过，测试实例和快照已删除。6 项失败／会话隔离单元回归通过。

数据库新增 `bindBrowserSandboxGateway`，将提供方 Session 和 HTTPS origin 在同一事务中绑定到当前启动操作。必须先绑定再允许 Agent sync，不能等 Agent 启动成功后才提供它启动所需的地址。旧操作、另一会话和已撤销节点不能回填；未绑定会话不能结算为 running。真实数据库回归覆盖这些边界。云端派发调用方和部署仍待接入。

## 派发前重新授权

`authorizeManualSandboxStart` 为手动接管派发提供事务内检查：部署开关、当前启动操作、节点和管理员状态、仍有效的用户会话、15 分钟内未取消的排队任务、账号启用及凭据版本、代理配置。仅全部符合时解密节点 Key，并核对现有哈希。调用方须在创建／恢复前检查，并在向 Agent 下发密钥前再次检查；结果不得返回页面或持久化到 Workflow 历史。

真实数据库测试覆盖旧操作、过期请求、无效会话、停用账号、凭据变更、取消任务、禁用管理员和撤销节点。现有发布／收件／登录数据库回归通过。这只是手动需求的授权入口；后台到期发布和收件必须接入各自的当前授权与调度逻辑，不能以手动任务的许可替代。自动派发仍待连接。

## 提供方派发边界

`provisionBrowserSandbox` 调用当前 SDK：首次启动从部署配置指定的已审核无凭据模板快照创建固定节点名称，2 vCPU，仅开放 9400，单次运行上限 20 分钟，保留最近一个快照（30 天存储期限）。正常空闲回收仍必须主动停止 VM，上限不是常驻许可。恢复先只读确认原持久实例已停止，再恢复同名实例；运行中、过渡状态、缺失快照或模糊错误都不新建替代实例。`inspectBrowserSandbox` 仅使用 resume:false。

`0036_browser_sandbox_dispatch.sql` 与 `claimManualSandboxDispatch` 在提供方 I/O 前持久化派发标识并标记已尝试初始化。相同操作只允许一次派发；首次创建响应丢失后不能重新走首次创建，必须查询原名称恢复。派发结果只含节点、操作和模式，不包含密钥。真实数据库验证 12 个并发派发请求只有一个赢家，重复领取被拒绝；4 项提供方夹具测试覆盖模板参数、只读检查、恢复、快照缺失和模糊响应，不代表提供方创建模板的实测。

当前仍没有调用上述函数的持久工作流，也尚未准备生产模板。创建／恢复与运行配置、授权复核、失败清理及正常停止的完整调度连接仍须完成。后台任务、实际 profile 和 WebSocket 接管验证也未完成。

## 单次派发控制器

`dispatchManualBrowserSandbox` 将数据库领取、提交后授权检查、提供方创建／恢复、网关绑定、凭据释放前再次授权、运行配置下发及 running 结果写入串成一次服务端执行。部署模板和镜像配置须先通过校验，不能占用派发后才发现配置缺失。正常结果只返回状态和会话 ID，不能把密钥或 SDK 对象写入工作流历史。

提供方返回会话之后的失败会尝试停止该确切会话，并保留 unknown 供后续核实；清理失败不能记成正常释放。提供方响应丢失时没有可靠会话对象，只记录待核实，不按名称盲目停止或再次创建。5 项控制器回归验证顺序、重复领取、授权撤销、网关／启动／结算失败及无法确认停止；使用可控依赖夹具，尚非完整数据库与云端集成测试。

持久工作流仍需负责触发此控制器、监测正常空闲退出、恢复 unknown 和调度剩余队列，生产模板也尚未准备。生产开关继续保持未启用。

## 持久监测工作流

`runManualBrowserSandbox` 新增一次手动启动操作的工作流：在服务端 step 调用派发控制器，成功后每 15 秒通过可挂起的 Workflow sleep 安排一次会话监测，最多 25 分钟覆盖提供方 20 分钟上限及停止延迟。不延长 Sandbox 超时，不在 VM 内保活。

`monitorBrowserSandboxSession` 先核对数据库会话，再用 resume:false 获取提供方状态。活动 Agent 继续运行，退出 Agent 通过已有回收函数停止整个会话；再次读取提供方确认 stopped 后才在事务中记录停止并清空网关地址。旧会话不能操作新会话。停止记录不释放浏览器租约或改写发布回执，恢复时仍须按原协议核实。

5 项监测单元回归通过。真实数据库回归验证停止记录、清空网关、保留任务队列及重复旧会话监测拒绝；原发布／登录／收件回归通过。工作流编译成功（3 workflows）；完整本地 Next 构建因共享 node_modules 符号链接超出 Turbopack 根目录而失败，完整构建交由 CI 验证，没有关闭 Turbopack 或修改编译器配置。

工作流尚未挂到用户入口和恢复调度。派发响应丢失／step 结果丢失的恢复、撤销后的及时停止、剩余队列唤醒、后台任务调度、生产模板与真实接管仍待完成。

## 撤销后回收（2026-09-11）

监测时重新读取节点状态和当前管理员权限／禁用状态。撤销时不等待 Agent 空闲：先核对同一 Session 和节点标签，给予 Agent 60 秒正常停止窗口，再清理残留容器、停止 Docker 和整台 VM。任何清理错误仍尝试停止已捕获会话，但不能报告干净释放。活动但未撤销节点仍走空闲检查。

14 项停止／监测回归通过，包括活动 Agent 撤销、错误节点和旧会话拒绝。真实数据库回归通过：禁用管理员后选择撤销路径、确认提供方停止后写入 stopped、清空网关且保留队列。提供方部分使用夹具，未执行真实账号撤销。类型检查通过。

上一提交 `b32cd02` 的完整 CI 生产构建已通过，验证新增 Workflow 可构建；浏览器预览测试仍发现重复 DOM 匹配。测试现限定可见表单并断言只有一份，仍需新 CI 结果，不把此前失败记为成功。生产接入与异常恢复仍未完成。

## 启动成功回执丢失恢复

重试先恢复已记录的会话，再按需读取新启动配置。即使部署配置缺失，现有会话仍能继续监测和回收；未启动的新操作仍须通过完整配置校验，且校验失败不领取派发、不创建实例。

派发控制器先读取同一 `dispatch_operation_id` 已记录的运行／停止中会话。若数据库已经记录启动成功而 Workflow step 回执丢失，重试返回该会话 ID，让工作流继续监测，不重新创建或恢复 VM、不下发密钥。领取竞争失败后也再查一次，覆盖另一执行者刚完成写入的情况。停止后及不匹配的操作不返回会话。

## 已绑定会话的异常启动回收

工作流遇到未确认的派发结果时也继续安排观察。`recoverBrowserSandboxDispatch` 对数据库已经绑定会话 ID 的异常启动执行回收：`unknown` 可立即处理，`starting` 必须至少五分钟未更新。事务先按节点、Sandbox 顺序加锁，核对原派发和操作 ID，再把原操作改为停止操作，阻止旧控制器继续绑定网关、领取密钥或写入启动成功。

回收只通过 `resume:false` 查询提供方并操作相同的已捕获会话。先尝试正常关闭 Agent 和浏览器；若启动失败导致 Agent 不存在或无法排空，仍停止该会话，随后读取提供方确认 `stopped`，才清空数据库会话和网关。队列、租约及发布结果不变，后续 Agent 仍须核实浏览器恢复情况。恢复中的停止操作不会被当成成功启动回执重放。

单元测试覆盖缺失 Agent、停止响应丢失、旧会话、过渡状态、已停止实例和并发启动成功；真实 PostgreSQL 覆盖未绑定拒绝、五分钟等待、旧控制器隔离、未知状态重入及队列保留。未绑定会话 ID 的提供方响应丢失仍待实现核实路径，不凭名称猜测会话，也不创建替代实例。本轮没有启动云端实例，生产开关继续关闭。

## 手动打开与持久派发入口

`0037_browser_sandbox_outbox.sql` 保存待派发的节点、操作 ID 和 Workflow 回执，不保存密钥或浏览器载荷。`ownerBrowserCommand(open)` 在保存任务的同一事务内重新校验当前手动需求，创建启动操作和 outbox。事务失败时三者一起回滚；未托管节点、空队列和不满足授权的需求不产生 Sandbox 启动请求。

Server Action 提交后立即尝试投递该节点的一个待办，仍只返回原有任务结果。投递失败不会把已提交的打开操作报告成失败。并发投递使用 `FOR UPDATE SKIP LOCKED` 和五分钟领取期限；Workflow 启动响应丢失后保留原操作 ID，期限过后允许重新投递，旧领取者不能覆盖新领取者的回执。允许重复 Workflow，但提供方创建／恢复仍由原来的数据库派发标识限制为一次。

`GET /api/browser-sandbox-dispatch` 提供每次最多十条的重试入口，要求配置 `CRON_SECRET` 并精确验证 Bearer 头；未授权或部署开关关闭时不加载投递模块。密钥传递约定见 [Vercel Cron 鉴权文档](https://vercel.com/docs/cron-jobs/manage-cron-jobs)。此提交未配置定时调用，未部署端点，也未通过真实 Workflow 启动完成入口验收；不能把持久记录等同于已经上线的自动补偿。后续还需接入定时触发、已取消且未派发操作的收尾，以及停止期间积累任务的下一轮唤醒。

真实 PostgreSQL 测试覆盖实际 owner open、事务回滚、十二个并发投递者、过期领取、旧回执、响应丢失后的延迟重投；端点测试覆盖未配置密钥、错误头和关闭开关。本轮未产生云端计算费用。

## 尚未派发的取消收尾

停止任务、禁用账号、轮换密钥或撤销节点后，平台在原事务中重新检查剩余手动需求。如果已经没有有效需求，且启动操作尚无提供方派发标识、也未绑定会话，就撤销该启动操作并恢复 `stopped`。派发时遇到失效用户会话也执行同样检查。保留旧 outbox 记录，迟到的 Workflow 不能复活已取消操作或取消后来新建的操作。

存在其他有效排队需求时保留启动操作。已经领取提供方派发的操作不能通过取消路径重置，必须继续按不确定结果恢复协议核实。真实 PostgreSQL 回归覆盖这些分支，包括先取消再重新打开的隔离。本轮未解决运行配置缺失导致 Workflow 无法进入派发检查、定时重试未部署及停止后下一轮唤醒的问题。

## 正常停止后的排队唤醒

运行会话的 Workflow 观察到终止后，在独立 step 中重新检查手动队列，生成下一轮 outbox 并尝试投递。停止事务先提交，后续凭据或投递失败不能撤销已确认的停止状态。观察回执丢失而返回 superseded 时也执行数据库检查；只有实际处于 stopped 且存在当前有效需求的节点才能创建下一轮启动，不凭旧会话回调启动重叠实例。

投递查询跳过已取消、已停止及不属于当前生命周期的旧操作，避免它们占用立即投递名额。PostgreSQL 回归确认停止后十二个并发唤醒仅产生一个新操作，下一次提供方领取为 resume，旧会话回调不影响新操作。异常启动回收仍不自动连锁重启，避免反复失败消耗费用。真实 Workflow 串联和生产调度仍待验证；最新已提交版本的发布预览 Playwright 在按钮点击处超时，不能将生产构建通过视作全部 CI 通过。

发布预览超时已定位为测试定位范围错误：提交按钮位于 CardFooter，通过 `form` 属性关联表单，并非表单子元素。回归改为定位可见的关联按钮，同时验证唯一性、可访问名称及原生 `button.form` 指向可见表单。仍保留旧预览提交被拒、数据库无发布记录、刷新后确认当前版本的断言；修正后的浏览器运行结果须由新 CI 验证。

恢复持久实例前还会检查 SDK 读取到的 timeout 配置，必须为有限正数且不超过 20 分钟；外部修改或缺失配置时拒绝 resume，不通过 update／extendTimeout 修复或延长当前计算。提供方单元回归覆盖无效、缺失、超限和较短时限。此检查是恢复前的配置检查，不保证外部操作者随后不能再次修改配置；正常空闲回收仍是停止计算的主要路径。

## 部署模板构建入口

`scripts/build-browser-sandbox-template.mts` 是显式运行的云端构建脚本，不在页面刷新、CI 或普通派发时执行。它只归档当前 Git 提交中的运行文件和固定 upstream 提交，在最长二十分钟的临时 VM 中构建 Agent／浏览器镜像并执行既有镜像冒烟测试。快照前要求没有任何容器和 Docker 卷、没有节点密钥或运行配置，并关闭 Docker daemon。

脚本保留一个七天到期的模板快照，删除构建 VM，并在独立的 ignored `tmp/browser-sandbox-template-<uuid>/result.json` 保存提交、上游版本、快照 ID 和不可变镜像 ID。快照存储可能计费；构建失败时删除临时 VM 及其孤立快照。模板成功生成也不自动启用生产，仍需用真实派发接口验证模板启动、恢复和浏览器会话持久化。当前只完成脚本格式及独立 TypeScript 检查，尚未运行该模板构建，不存在可声称已部署的新模板。

### 首次模板构建实测

提交 `cdd77f8` 已实际完成模板构建：Docker/cgroup 初始化、Agent／浏览器镜像构建、镜像冒烟、无容器和无账号卷检查、Docker 停止均通过，生成七天有效模板快照后删除了构建 VM。证据保存在 ignored `tmp/browser-sandbox-template-18f72bfb-00bb-4648-80df-3ada4983c9f7/result.json`，包含快照与不可变镜像 ID；没有启用生产或配置账号。模板恢复及真实浏览器 profile 一致性尚未验证。

同一提交的 [Playwright 运行 34683613726](https://github.com/ban12-project/F-Trade-Platform/actions/runs/34683613726) 已成功完成，确认此前发布预览的表单外按钮定位修复通过浏览器回归。该结果证明此测试已恢复，不代替 Sandbox 真实 Workflow 或生产接管验收。

### 模板创建与恢复实测

`scripts/test-browser-sandbox-template.mts` 已通过实际 `provisionBrowserSandbox` 从上述模板创建持久实例，并通过同一提供方入口恢复。初始镜像和无卷检查、镜像冒烟、合成卷写入、整机停止、resume:false 状态查询及二十分钟配置检查通过；恢复后的会话 ID 不同，合成卷标记和浏览器镜像仍在。测试 VM 及其孤立测试快照已删除，原模板保留。证据位于 ignored `tmp/browser-sandbox-template-test-8c72d095-f6ec-4178-af0b-cd593a0e58d5/result.json`。

该测试仅证明模板派发、Docker 与合成卷恢复，没有使用账号、代理或真实浏览器 profile，不能据此声称登录状态、WebSocket 接管和生产 Workflow 已验收。

后续增加了真实浏览器 cookie／localStorage 合成回归：通过 example.com 页面写入数据，停止 VM 后尝试重新读取。两次实测均在创建标签页时返回 HTTP 500，尚未进入持久化成功路径；错误响应仅给出 Internal server error。两次测试 VM 和孤立快照均已删除。服务 health 和镜像冒烟通过不能证明真实标签页可创建，此问题是当前部署阻塞。IndexedDB 在运行配置中关闭，亦未证明真实账号登录状态可恢复。

用户更换数据库后，已对本地 `.env` 指向、初始无业务表的 Neon 数据库应用当前分支全部 38 条迁移，并按迁移 ID 对照本地文件哈希核实一致。初始化后用户、Sandbox、outbox 记录均为零；未迁移旧数据、未创建管理员、未启用 Sandbox。Vercel production 的 DATABASE_URL 为不可下载明文的敏感配置，尚未证明线上部署已连接同一数据库。

进一步合成诊断在浏览器启动阶段观察到 180 秒握手超时，日志包括只读 `/root/.cache/dconf`、Fontconfig 无可写缓存、profile 目录异常和缺失 `glxtest`。尚未确定哪一项是直接原因，不能声称修复。证据在 ignored `tmp/browser-sandbox-template-test-3f15f9c4-c063-4cfc-a2f6-785109bfa30e/browser-profile-write.log`；该 VM 及孤立快照由测试清理，随后提供方查询返回 not_found。诊断夹具新增独立进程硬期限及外层 timeout，防止仅依赖 SDK 命令超时导致诊断运行过久。

可写缓存对照实验将 XDG cache/config 指向 `/tmp`，并显式指定原浏览器二进制。Fontconfig／dconf 写入警告消失，但 API 仍返回 500，profile 目录异常及二十秒 headless 握手超时仍在。因此缓存不可写不是足以解释或修复问题的原因；没有把实验设置推广到生产。证据位于 ignored `tmp/browser-sandbox-template-test-f2773a50-4f4a-4a3c-b70e-e63621779b07/browser-profile-write.log`，测试 VM 与快照已删除。诊断中的原生启动现在另设二十秒 timeout，下一步核查固定发行包和启动协议兼容性。

依据 [Camoufox #572](https://github.com/daijro/camoufox/issues/572)，对照试验仅新增 `/root/.camoufox` 的 16 MiB tmpfs，保持只读根文件系统。真实标签页创建及合成 cookie／localStorage 写入首次通过，故将该目录挂载加入 Agent 容器规格与镜像冒烟配置；账号存储仍由 persistence 插件写到 `/data`。已有模板中的旧 Agent 镜像不包含此修复，部署前必须重建模板。

该次测试恢复 VM 后在 cgroup 初始化遇到 Device or resource busy，未执行恢复后的 cookie 读取。不能把首次写入通过记作完整持久化通过。证据在 ignored `tmp/browser-sandbox-template-test-3c075242-97b1-456e-8099-0c51bcf4ab22/`，测试 VM 和孤立快照已删除；当前下一阻塞是恢复阶段的 cgroup 初始化。

6 项控制器回归及真实 PostgreSQL 回归通过，验证已记录会话重放、错误操作拒绝和停止后拒绝。该路径只解决数据库已经成功记录的情况；提供方创建响应丢失或尚未写入 running 的操作仍需核实与恢复，不能宣称 unknown 恢复已完成。

### 真实浏览器恢复验证通过

cgroup 初始化保留五次尝试上限，在失败的尝试之间等待一秒，总等待最多四秒；耗尽后记录控制器和进程编号以便排查。一次成功不足以证明恢复竞争已完全消除，失败仍会终止启动。

采用当前初始化脚本与 `/root/.camoufox` 临时目录挂载后，合成测试完成真实标签页创建、cookie／localStorage 写入、整机停止、恢复和读取，两个浏览器状态断言均通过。结果位于 ignored `tmp/browser-sandbox-template-test-676101ed-d748-450e-a87e-e27859a1f10b/result.json`，其中 `actualBrowserProfileVerified` 与 `cleanedUp` 均为 true。测试实例及孤立测试快照已删除。此次未测试 IndexedDB、真实账号登录、代理、noVNC 或生产 Workflow。

该验证在旧模板上临时上传当前初始化脚本，并显式创建修复后挂载规格的浏览器容器；旧模板 Agent 镜像仍须重建，不能把此结果作为生产模板已更新的证据。提交 `63d1622` 的 CI 构建、Playwright、数据库和双架构镜像冒烟均通过；Vercel 构建被 Ignored Build Step 取消，不代表已部署预览。

再次只读核对新数据库：38 条迁移哈希与当前分支一致，用户、Sandbox、outbox 记录仍均为零。生产数据库连接身份尚未核实。

### 修复版模板交付验证

已从运行提交 `8109b93` 重建模板，证据位于 ignored `tmp/browser-sandbox-template-f99ff5cf-555d-49ff-99d0-5c7bae5469b7/result.json`。新 Agent 镜像包含只读根文件系统下的 `/root/.camoufox` 临时挂载，模板内初始化脚本包含有界 cgroup 重试。镜像冒烟、无账号卷检查通过，构建 VM 已删除。

提交 `250c9a1` 的测试直接使用模板内脚本，不再上传替代初始化脚本。真实浏览器 cookie／localStorage 在写入、整机停止及恢复后读取全部通过，结果位于 ignored `tmp/browser-sandbox-template-test-d23ac3b2-c520-48ad-abab-e508f6ae795c/result.json`；测试 VM 与孤立测试快照均已删除。该结果仍不覆盖实际 Agent 派发、真实登录、代理、noVNC 或生产 Workflow。

新模板验证后，已删除未用于生产的旧模板 `snap_e9WUZREfFGXUKsAuTM5F5WKAyY7f`；保留新模板的七天到期设置，清理回执在 ignored `tmp/retired-browser-template.json`。生产开关保持关闭。

### 模板内 Agent 启动与空闲停止

`scripts/test-browser-sandbox-start.mts` 现在可接收模板构建结果，直接使用快照内脚本及不可变 Agent／浏览器镜像，不上传替代运行源码或重新构建 Agent。合成 broker 只返回空任务。新模板实测通过私密运行配置下发、重复启动返回 already-running、临时文件清理、512 MiB 内存限制、HTTPS viewer 静态入口、空闲正常退出、同一操作不会重启已退出 Agent，以及提供方确认整机 stopped。

证据位于 ignored `tmp/browser-sandbox-start-cbc08017-c16d-4241-b068-de1d1cee91d1/result.json`，记录模板与测试提交版本。测试 VM 和快照已删除。此测试没有真实任务，不覆盖 PostgreSQL outbox 到生产 Workflow 的投递、浏览器 WebSocket 或代理；HTTPS 静态入口通过不等于 noVNC 接管已通过。

### 派发重试调度配置

`vercel.json` 配置每五分钟调用 `/api/browser-sandbox-dispatch`，为即时投递失败或回执丢失后的 outbox 提供后续触发。每次最多领取十条，领取期限仍为五分钟；需求已过期时派发重新授权会拒绝启动。调度不创建空队列任务、不直接创建或恢复 Sandbox。

已只读核实当前 ban12 团队套餐为 Pro。Vercel [套餐限制](https://vercel.com/docs/cron-jobs/usage-and-pricing)支持该频率，[Cron 仅在生产部署生效](https://vercel.com/docs/cron-jobs/quickstart)，预览通过不能证明调度已运行。部署必须配置 `CRON_SECRET`，端点使用 [Vercel Bearer 约定](https://vercel.com/docs/cron-jobs/manage-cron-jobs)校验；开关关闭时不读取待办或启动 Workflow。此提交仅补齐部署配置，尚未部署、未启用生产。

调度函数本身仍计入 Functions 使用量；每五分钟约每天 288 次调用，并非零费用。正常打开仍立即投递，调度仅承担重试，不改变浏览器按需启动和空闲整机停止约束。

本轮只读查询 Vercel 项目变量元数据，确认 `CRON_SECRET`、`BROWSER_SANDBOX_ENABLED`、`BROWSER_SANDBOX_TEMPLATE_SNAPSHOT_ID` 尚未配置；`DATABASE_URL` 在 production／preview 中为 sensitive 类型，仍未核实其连接身份。未输出变量值。

### 真实 VNC 的公网 WebSocket 握手

`scripts/test-browser-sandbox-websocket.mts` 使用修复版模板启动真实浏览器和 VNC，再由模板中的网关代码代理 noVNC 模块与 WebSocket。合成 broker 只批准测试票据，不访问平台用户或真实账号。实测通过公网 HTTPS 读取 noVNC 模块，经 TLS WebSocket 收到真实 VNC 的 RFB 协议问候；同一 WebSocket 能力地址再次使用被拒绝。

证据位于 ignored `tmp/browser-sandbox-websocket-55dff74a-7df7-4b2b-96ea-ccbb273fffd4/result.json`，测试 VM 和快照已删除。此验证只覆盖传输握手与重复使用拒绝，未进行 VNC 密码认证、桌面渲染或交互，亦未验证平台票据签发、代理出口和真实账号接管。测试实例最长五分钟，不改变生产关闭状态。

### noVNC 密码认证与实际画面

云端合成测试现使用本地 Chromium 打开模板内真实 viewer 页面，经允许来源的 postMessage 交付独立测试票据，执行 noVNC 密码认证。首次仅检查 canvas 尺寸时截图仍为空白，不能证明画面已到达；回归因此增加等待非单色像素更新，避免把连接成功当成桌面渲染成功。

严格复测通过，人工检查截图可见远端 Firefox 的 Example Domain 页面。证据位于 ignored `tmp/browser-sandbox-websocket-d32f2efc-3ea1-475c-bc9c-ebfcb083de24/`，包含 `desktop.png` 与清理后写入的 `result.json`。首次及复测的 VM、快照均已删除。本次未测试键鼠输入、实际平台父页面及票据签发、代理出口或真实账号，不把画面可见等同于完整接管验收。

### 真实 noVNC 键鼠输入

合成测试通过真实 viewer canvas 发送鼠标点击和 Control+L／键盘导航，随后在远端浏览器内读取 DOM 点击标记与 location.hash，确认操作实际到达远端。截图中远端地址栏可见 `#ftrade-keyboard-proof`，与断言一致。观察夹具仅安装点击监听并读取结果，没有通过浏览器 API 代替键鼠动作。

证据位于 ignored `tmp/browser-sandbox-websocket-27c76842-b7d3-47ed-97bc-0d3bfb0f0219/`，包含 `desktop-input.png` 与清理完成后的 `result.json`。测试 VM 和快照已删除。已覆盖模板内网关、认证、画面和基础键鼠传输；实际平台父页面、票据签发、生产 Workflow、数据库连接身份、代理及真实账号链路仍未完成。

### 平台托管节点创建入口

浏览器管理页新增托管创建卡片，只填写名称；服务端将启用开关作为布尔展示状态传给客户端，实际 Server Action 仍独立检查会话、管理权限、输入和后端开关。客户端与命令契约复用 `sandboxNodeFormSchema`。创建成功提示尚未启动浏览器，不索取网关和接入密钥；列表刷新失败不会把已经创建的节点报告为创建失败。原自管服务器入口保留。

两项 Playwright 回归验证关闭状态不可提交、空名称校验，以及真实 Action 请求只包含 create-sandbox 和整理后的名称；请求被拦截，未进行业务写入。隔离 Next.js 16.3.2 Turbopack 实例的编译与运行错误均为空，React 树与截图确认卡片正常渲染。测试页仅在 NEXT_ENABLE_TESTING_API=1 时可用。没有启用生产或创建云端实例，托管创建 UI 的存在不代表整条生产派发链路已验收。

### 首次创建响应丢失后的回收

未绑定会话的未知启动现在可通过只读提供方查询核实首次创建：节点名称、持久实例属性、ftrade-node 和 ftrade-created-by 标签必须匹配当前节点与派发操作，会话历史必须恰好一条且与当前会话一致。只凭名称、标签缺失、多会话历史或当前会话不一致均不能进入回收；恢复操作的响应丢失仍需另行核实。

核实后在事务中重新检查当前未绑定操作，只有 unknown 或至少五分钟未更新的 starting 可以写入发现的会话并转为停止操作。并发只有一方能绑定，旧控制器不能再绑定网关或报告启动成功。沿用已有会话回收，读取提供方确认停止后才清空生命周期记录，不释放业务租约或自动重新创建。

15 项提供方／恢复回归、类型检查和真实 PostgreSQL 并发测试通过。另在真实 Vercel 创建后模拟客户端丢失响应，正确操作可发现唯一会话、错误操作被拒绝，测试 VM 和快照随后删除；证据位于 ignored `tmp/initial-discovery-26980274-fb47-49df-8767-7742ba11f3ed.json`。云端试验验证提供方发现能力；数据库限制与回收编排由分别对应的数据库及单元回归验证，不将其描述为完整生产 Workflow 演练。
