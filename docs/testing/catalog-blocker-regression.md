...
用户随后明确授权补充 PDF 缺失信息并继续。已按[reference mock 后续流程](reference-mock-workflow.md)执行 20 条测试副本：20 条测试 Ready、20 条模拟商机流程、280 次状态转换。缺少工厂资料不再阻断这条 mock 测试路径。以下 Ready 为 0 和补充来源要求仅描述保留原始事实的诊断，不是继续 mock 测试的前置条件。

## 原始资料诊断（基线 4eba76c，2026-09-05）
