...
The subsequent 2026-09-25 consistency check used a temporary fork of the stopped production Sandbox, its actual Docker policy (without the local Podman filter), the same production browser image, and its preserved account volume. Two sequential containers with distinct task IDs each verified proxy egress and account identity and returned the login plugin's `ready` state with zero factor submissions. The fork was stopped and deleted. A separate read-only production database check found the latest application interactive task completed with saved-login outcome `ready`, no credential claim/release, and the node stopped with no queued/live task. This preserves the distinction between the new fork observation and the existing application receipt; it is not a newly executed broker task, video receipt, or inbound DM test.
