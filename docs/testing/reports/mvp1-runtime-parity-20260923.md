...
The managed compatibility hook now disables `media.peerconnection.enabled` for the MVP1 text-only browser, preventing WebRTC from opening a separate direct path around the HTTP proxy. This behavior is covered by the launch-hook unit test and a real Firefox profile preference check in the image smoke test. It does not yet prove a network-level no-bypass result for all traffic; a proxy-down negative browser test and Vercel Sandbox observation remain outstanding.
