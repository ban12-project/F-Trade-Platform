...
- 原运行 CAPTCHA 人工交接已实现，并通过合成执行器、数据库及实际 UI 回归。后续真实恢复测试曾在一次授权内完成密码/TOTP/PIN，但这仅证明恢复流程，不能替代会话持久化。依据用户纠正，停止反复创建全新会话；#424 保持 draft，生产工作台的只读核对见下文。
...
- 未打补丁的固定上游 `79d425be26743883a06613eaa3be5e38e7ab5409` 的 `server.js` 在 `session:creating` 钩子后固定调用 `newContext(contextOptions)`；持久化插件只注入 storageState。启用 IndexedDB 或修改目录配置无法实现原生 profile 生命周期，需要独立修改会话创建及关闭契约，并验证账号隔离、租约回收和回滚。生产配置未改变。
- #425 已实现显式迁移和账号独立原生 profile，并保留旧快照；本地同一真实账号在一次迁移 PIN 恢复后，跨任务与容器删除重建均无需密码/TOTP/PIN，页面账号标识和 Messenger 列表已分别核对。两次停止均退出 0。该证据不证明历史恢复或真实 DM 采集；详细记录在 #425 分支的 native-profile 报告。
- #424 的 `0a03667` 已将执行器改为先观察会话、仅在明确恢复阶段领取凭据。已就绪与 CAPTCHA 人工处理后直接就绪的浏览器测试均为零凭据领取；数据库就绪回执允许自动授权未领取凭据的路径，并重新检查当前授权。撤销授权拒绝测试通过。该改动与 #425 尚未合并部署，不能写成生产能力已验收。
...
| 社交会话连续性 | 本地同一账号跨任务、容器删除重建后身份匹配，Messenger 可用且无需再次提交因子；先观察再领取凭据的执行器与数据库测试通过 | 合并后的同一运行镜像/节点联调与生产接入；历史恢复另验，CAPTCHA 不得导致循环登录 |
