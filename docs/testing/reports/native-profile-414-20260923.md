...
## Real-account local continuity observation

Using the original account scope, fixed approved proxy and explicitly migrated volume, the browser observed the expected egress and reached the authenticated Facebook UI. The first Messenger visit required one previously authorized PIN recovery; no password or TOTP was submitted. Verification completed and the Messenger thread-list UI appeared.

The task session was then closed through the API. A subsequent task on the same native profile again reached Messenger without password, TOTP or a PIN prompt. The container was stopped with exit code 0, deleted and recreated with the same data mount. The new container again observed the expected egress and reached Messenger without submitting any factors. A separate read-only page evaluation verified Facebook origin and every nonzero `CurrentUserInitialData` account identifier against the reviewed account reference, found the thread-list navigation, and found no PIN input.

Private snapshots and observation records remain outside the repository. These observations establish local login and Messenger UI continuity for this account and runtime. They do not prove historical chat restoration, inbound ingestion/deduplication, production rollout or session-first task gating. No message or publication was sent. The first migration PIN restoration is recovery setup, not counted as a prompt-free continuity step. The local debug launcher retained a bounded ten-minute test lifetime and renewed the standard short lease atomically.
