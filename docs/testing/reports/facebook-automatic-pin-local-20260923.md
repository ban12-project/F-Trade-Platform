...
独立本地容器和独立迁移数据库的全新会话，通过固定出口检查、密码单次提交、TOTP 单次提交及账号身份核对，进入 Messenger 加载阶段；随后约在领取凭据后 85 秒返回 refused，未取得 ready。服务端授权当时尚未到期。初始准备过程中还分别遇到测试数据库唯一绑定约束、测试脚本漏传等待回调及调试基础镜像的零字节 GeoIP 文件；这些均发生在凭据领取之前。原生产工作台会话未改动。

根因是执行器将自动登录授权交给 `localDeadline` 换算，该函数把所有期限限制为 90 秒，再扣除 RTT 和 5 秒余量；此前服务端 180 秒窗口没有完整传到执行器。新增独立的自动登录授权换算（上限 180 秒），保留 RTT/安全余量；容器租约仍使用原 90 秒换算并由运行时逐步检查。旧版人工填充未变。

定向浏览器回归 3/3 通过，覆盖执行器收到超过 90 秒的授权、人工 checkpoint 交接和失效租约拒绝；新增时钟偏移、RTT、180 秒上限和即将过期拒绝的确定性测试。既有节点 29 项回归通过。此修正后的全新真实会话仍待复测，不能以本次密码/TOTP 成功推断 PIN 或整体 ready。


## 修正后同轮密码、TOTP、PIN 与持久回执通过

修正后的第一次全新会话出现真实 CAPTCHA；节点成功上报 checkpoint，在原授权窗口内等待，人工未完成时到期返回 refused，一次领取、一次终态回执。随后用户反馈 VNC 超时，确认这是本轮授权到期后关闭会话。没有将超时改写成成功，也没有延长原授权。

使用同一本地账号资料开启新运行（保留上一轮 checkpoint 后的浏览器资料，因此不宣称磁盘完全空白），页面仍从密码登录开始。本轮未出现 CAPTCHA；正式本地 broker→一次性加密凭据领取→密码→TOTP→Messenger 导航→PIN→ready 全部在同一次新授权内完成。日志记录密码、TOTP、导航、PIN 各提交一次，账号身份核对通过；执行器报告 claims=1、receipts=1、storedOutcome=ready。
