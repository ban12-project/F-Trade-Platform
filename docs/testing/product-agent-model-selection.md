...
发布前执行 `pnpm exec tsx scripts/validate-product-model-selection-report.ts <summary.json>`：它要求完整且测量有效的正式批次，从试次重算统计，验证运行健康，并拒绝未知字段。`test:product-output` 也会复核已提交的选型 JSON 报告。
