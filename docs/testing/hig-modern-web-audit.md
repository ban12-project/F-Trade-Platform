...
- After those test changes, all ten UX cases passed in both Chromium and Playwright WebKit 26.5. The optional WebKit configuration runs the acceptance file serially: a concurrent two-worker run missed the transient 1.5-second loading state (9/10 passed), while three isolated hydrated-navigation probes observed the RSC request and loading state. This is a test scheduling limitation, not a claim of parallel WebKit stability. A first WebKit attempt also lost its reused server when the Chromium runner exited; those connection failures were infrastructure failures, and the server was then kept alive independently.
