...
export function ProductRevisionPanel({ product }: { product: ProductCatalogDetail }) {
  ...
  if (product.state !== "PRODUCT_REVISION_REQUIRED") {
    return <div className="mx-auto min-h-full w-full max-w-4xl p-4 md:p-6 lg:p-8"><Alert><AlertTitle>当前无需修订</AlertTitle><AlertDescription>该产品未处于待修订状态。</AlertDescription></Alert></div>;
  }

  return (
    <div className="mx-auto flex min-h-full w-full max-w-4xl flex-col gap-6 p-4 md:p-6 lg:p-8">
      <header className="flex flex-col gap-3">
        <LinkButton className="w-fit" size="sm" variant="ghost" href={`/console/products/${product.id}`}><ArrowLeftIcon data-icon="inline-start" />返回审核</LinkButton>
        <h1 className="text-3xl font-semibold tracking-tight text-balance">修订产品草稿</h1>
        ...
        <CardContent>
          <form autoComplete="off" onSubmit={form.handleSubmit(onSubmit)}>
            <FieldGroup>
              ...
              <div className="flex justify-end"><Button type="submit" disabled={pending}>{pending ? <Spinner aria-hidden="true" data-icon="inline-start" /> : <RotateCcwIcon data-icon="inline-start" />}提交修订并送审</Button></div>
            </FieldGroup>
          </form>
        </CardContent>
