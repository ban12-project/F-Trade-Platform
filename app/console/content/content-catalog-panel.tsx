...
import { ClapperboardIcon, FilePenLineIcon, ShieldCheckIcon } from "lucide-react";
...
export function ContentCatalogPanel({ entries, products, canReview }: { entries: ContentCatalogEntry[]; products: ReadyProductContentSource[]; canReview: boolean }) {
  ...
  return <div className="mx-auto flex min-h-full w-full max-w-7xl flex-col gap-6 p-4 md:p-6 lg:p-8">
    <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div className="flex flex-col gap-2"><div className="flex flex-wrap gap-2"><Badge variant="secondary">营销内容 · 图文</Badge><Badge variant="outline">渠道尚未启用</Badge></div><h1 className="text-3xl font-semibold tracking-tight text-balance">创建内容草稿并进行事实审核</h1><p className="max-w-3xl text-muted-foreground text-pretty">只能引用已审核的产品字段。正式发布前，人工必须确认官方渠道；此处不能发送内容。</p></div><LinkButton variant="outline" href="/console/video"><ClapperboardIcon data-icon="inline-start" />创建视频计划</LinkButton></header>
