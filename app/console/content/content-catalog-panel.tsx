...
export function ContentCatalogPanel({ entries, products }: { entries: ContentCatalogEntry[]; products: ReadyProductContentSource[] }) {
  ...
  return <div className="mx-auto flex min-h-full w-full max-w-7xl flex-col gap-6 p-4 md:p-6 lg:p-8">
    ...
    {products.length === 0 ? <Empty><EmptyHeader><EmptyMedia variant="icon"><FilePenLineIcon /></EmptyMedia><EmptyTitle>暂无可用的 Product Ready</EmptyTitle><EmptyDescription>先在产品工作台完成 Gate 01；内容工作台不会以草稿或推测数据起草内容。</EmptyDescription></EmptyHeader><EmptyContent>这不是故障：当前环境尚未导入获授权的真实 SKU。</EmptyContent></Empty> : <Card><CardHeader><CardTitle>新建待审内容</CardTitle><CardDescription>渠道固定为“待渠道决策”，不会产生任何对外发布。</CardDescription></CardHeader><CardContent><form autoComplete="off" onSubmit={form.handleSubmit(onSubmit)}><FieldGroup>
      <Field data-invalid={!!form.formState.errors.productId}><FieldLabel htmlFor="content-product">已核验产品</FieldLabel><Controller control={form.control} name="productId" render={({ field }) => <Select value={field.value} onValueChange={(value) => { field.onChange(value); form.setValue("factPath", ""); }}><SelectTrigger id="content-product" className="w-full"><SelectValue placeholder="选择 Product Ready…" /></SelectTrigger><SelectContent><SelectGroup>{products.map((product) => <SelectItem key={product.id} value={product.id}>{product.internalSku} · {product.productName}</SelectItem>)}</SelectGroup></SelectContent></Select>} /><FieldError errors={[form.formState.errors.productId]} /></Field>
