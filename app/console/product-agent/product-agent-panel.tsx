...
export function ProductAgentPanel({ configured }: { configured: boolean }) {
  ...
  return <div className="mx-auto flex min-h-full w-full max-w-4xl flex-col gap-6 p-4 md:p-6 lg:p-8">
    <header className="flex flex-col gap-2"><div className="flex items-center gap-2 text-sm text-muted-foreground"><BotIcon aria-hidden="true" /><span>产品资料结构化</span></div><h1 className="text-3xl font-semibold tracking-tight">Product Agent</h1><p className="max-w-3xl text-muted-foreground">将已授权且脱敏的工厂资料文本转换为待审核产品草稿。结果不会自动成为 Product Ready。</p></header>
