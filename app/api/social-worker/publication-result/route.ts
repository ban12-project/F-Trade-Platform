...
export async function POST(request: Request) {
  try {
    ...
    const saved = await recordControlledPublicationResult({
      jobId: result.jobId,
      outcome: result.outcome,
      externalPublicationRef: result.externalPublicationRef,
      failureCode: result.failureCode,
    });
    return NextResponse.json({ publicationId: saved.id, status: saved.status });
  } catch {
    return NextResponse.json({ error: "invalid_publication_result" }, { status: 400 });
  }
}
