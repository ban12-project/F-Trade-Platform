...
export async function POST(request: Request) {
  const reply = (active: boolean, status = 200) => Response.json({ active }, { status, headers: { "Cache-Control": "no-store" } });
  try {
    ...
    try { for (;;) { const { done, value } = await reader.read(); if (done) break; size += value.byteLength;
      if (size > 128_000) { await reader.cancel(); return reply(false, 413); } chunks.push(value); }
