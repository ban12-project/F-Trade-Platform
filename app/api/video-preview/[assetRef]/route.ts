...
export async function GET(request: Request, context: { params: Promise<{ assetRef: string }> }) {
  ...
  const preview = await resolveWorkspacePrivateVideoPreview(session, assetRef, undefined, undefined, request.headers.get("range"));
  ...
  return new Response(preview.asset.body, { status: preview.asset.contentRange ? 206 : 200, headers: privateVideoPreviewHeaders(preview.asset) });
}
