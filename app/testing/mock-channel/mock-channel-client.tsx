    ...
    <main id="main-content" className="flex min-h-svh items-center justify-center bg-muted/30 p-4 md:p-8">
      <Card className="w-full max-w-2xl">
        <CardHeader>
          ...
          <CardTitle><h1>Mock official channel 验收</h1></CardTitle>
          <CardDescription>仅测试构建可用：只处理 synthetic webhook 元数据，不发送真实 API 请求。</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-5">
          <div className="grid gap-2 sm:grid-cols-2">
            <Button type="button" variant="outline" onClick={() => void invoke("reset")} disabled={pending}>
            ...
            <Button type="button" variant="outline" onClick={() => void invoke("duplicate")} disabled={pending}>重发相同 webhook</Button>
            <Button type="button" variant="outline" onClick={() => void invoke("fail")} disabled={pending}>模拟下游失败</Button>
            <Button type="button" variant="outline" onClick={() => void invoke("retry")} disabled={pending}>重试失败 webhook</Button>
          </div>
          <output className="rounded-lg border bg-muted/30 p-4 text-sm leading-6" aria-live="polite">
