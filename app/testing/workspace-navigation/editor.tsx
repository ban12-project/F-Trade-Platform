...
import { useWorkspaceDirty, WorkspaceDirtyProvider } from "@/components/workspace/dirty-state";
...
function Editor({ href }: { href: string }) {
  ...
  return (
    <div className="space-y-4 p-6">
      <Label htmlFor="navigation-draft">测试草稿</Label>
      <Input
        id="navigation-draft"
        value={value}
        onChange={(event) => setValue(event.target.value)}
      />
      <WorkspaceLink href={href} prefetch={false}>
        切换测试页面
      </WorkspaceLink>
    </div>
  );
