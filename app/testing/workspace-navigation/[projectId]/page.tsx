...
async function Content({ params, searchParams }: Props) {
  ...
  return <ProjectWorkspace project={navigationProject} tasks={[]} stages={marketingStages} activeStage="product" basePath={`/testing/workspace-navigation/${navigationProject.id}`} panel={<NavigationEditor href="/testing/workspace-navigation" />} />;
}
export default function Page(props: Props) {
  if (process.env.NEXT_ENABLE_TESTING_API !== '1') notFound();
  return <Suspense fallback={<WorkspaceLoadingSkeleton project />}><Content {...props} /></Suspense>;
}
