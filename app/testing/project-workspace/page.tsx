...
async function ProjectWorkspaceFixture({ searchParams }: { searchParams: Promise<{ panel?: string }> }) {
  const query = await searchParams; const active = stages.some((stage) => stage.id === query.panel) ? query.panel! : "product"; const stage = stages.find((item) => item.id === active)!;
  const panel = <Card><CardHeader><CardTitle>{stage.label}记录</CardTitle><CardDescription>合成测试详情面板</CardDescription></CardHeader><CardContent>人工操作保持可追溯。</CardContent></Card>;
  const membersPanel = <ProjectMembersPanel projectId={project.id} currentUserId="synthetic-user-owner" members={[
    { userId: "synthetic-user-owner", name: "Synthetic Owner", email: "owner@example.test", role: "owner" },
    { userId: "synthetic-user-viewer", name: "Synthetic Viewer", email: "viewer@example.test", role: "viewer" },
  ]} />;
  return <ProjectWorkspace project={project} projects={[project]} tasks={tasks} membersPanel={membersPanel} stages={stages} activeStage={active} panel={panel} basePath="/testing/project-workspace" />;
}

export default function ProjectWorkspaceTestingPage({ searchParams }: { searchParams: Promise<{ panel?: string }> }) {
  if (process.env.NEXT_ENABLE_TESTING_API !== "1") notFound();
  return <Suspense fallback={null}><ProjectWorkspaceFixture searchParams={searchParams} /></Suspense>;
}
