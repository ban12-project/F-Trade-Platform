...
import { transitionFrames } from "./abcd-types";
...
function Beat({ clip, productName, ctaText }: { clip: AbcdMarketingVideoProps["clips"][number]; productName: string; ctaText: string }) {
  ...
  const hasBranding = clip.abcdRoles.includes("branding");
  const hasConnection = clip.abcdRoles.includes("connection");
  const hasDirection = clip.abcdRoles.includes("direction");
  const attentionFlash = clip.abcdRoles.includes("attention")
    ? interpolate(frame, [0, 4, 14], [.72, .15, 0], { extrapolateRight: "clamp" })
    : 0;
  ...
  return <AbsoluteFill style={{ backgroundColor: "#09090b", overflow: "hidden", fontFamily: "Arial, Helvetica, sans-serif" }}>
    ...
    <div style={{ position: "absolute", left: 74, right: 74, top: 72, display: "flex", alignItems: "center", justifyContent: "space-between", opacity: hasBranding ? enter : .88 }}>
    ...
    <div style={{ position: "absolute", left: 74, right: 74, bottom: hasDirection ? 214 : 108, transform: `translateY(${(1 - enter) * 42}px)`, opacity: enter }}>
      {hasConnection ? <div style={{ color: accent, fontSize: 24, fontWeight: 800, letterSpacing: 2.2, marginBottom: 18 }}>FOR DISTRIBUTOR CONVERSATIONS</div> : null}
