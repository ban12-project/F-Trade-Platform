...
const rules: readonly TransitionRule[] = [
  ...
  },
  {
    entityType: "product",
    fromState: "PRODUCT_REVIEW_REQUIRED",
    toState: "PRODUCT_REVISION_REQUIRED",
    actors: ["human"],
    gate: "gate_01_truth",
  ...
  },
  {
    entityType: "content",
    fromState: "CONTENT_REVIEW_REQUIRED",
    toState: "CONTENT_REVISION_REQUIRED",
    actors: ["human"],
    gate: "gate_01_truth",
    approvalStatus: "rejected",
  },
  ...
  { entityType: "video", fromState: "VIDEO_DRAFT", toState: "VIDEO_RENDERING", actors: ["human"] },
  {
    entityType: "video",
    fromState: "VIDEO_REVISION_REQUIRED",
  ...
  {
    entityType: "video",
    fromState: "VIDEO_APPROVED",
    toState: "VIDEO_READY_FOR_GENERATION",
    actors: ["human", "system"],
  },
  {
    entityType: "video",
    fromState: "VIDEO_READY_FOR_GENERATION",
    toState: "VIDEO_DRAFT",
    actors: ["agent", "human"],
  },
  {
  ...
  },
  {
    entityType: "quotation",
    fromState: "QUOTE_REVIEW_REQUIRED",
    toState: "QUOTE_REVISION_REQUIRED",
    actors: ["human"],
    gate: "gate_02_quote",
    approvalStatus: "rejected",
  },
  ...
  { entityType: "lead", fromState: "OPPORTUNITY", toState: "LOST", actors: ["human"] },
  {
    ...
    toState: "DELIVERY_CONFIRMATION_CONFIRMED",
    actors: ["human"],
  ...
  },
  {
    entityType: "delivery_confirmation",
    fromState: "DELIVERY_CONFIRMATION_PENDING",
    toState: "DELIVERY_CONFIRMATION_REJECTED",
    actors: ["human"],
    gate: "gate_03_delivery",
    approvalStatus: "rejected",
  },
...
export function assertTransition(event: WorkflowEventInput, approval?: ApprovalDecision): void {
  ...
  if (new Set(event.evidenceRefs).size !== event.evidenceRefs.length)
    fail("evidenceRefs must be unique");
  ...
  if (!rule.actors.includes(event.actorType))
    fail(`${event.actorType} cannot perform this transition`);

  if (!rule.gate) {
    if (event.gate || event.approvalRef || approval)
      fail("this transition must not carry a Human Gate approval");
    return;
  }
...
export function replayTransitions(
  ...
  for (const { event, approval } of entries) {
    ...
    if (event.fromState !== state)
      fail(`event ${event.eventId} expected ${state}, received ${event.fromState}`);
