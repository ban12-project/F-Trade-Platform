...
export function decideDeliveryConfirmation(
  ...
  decision: { actorType: "human" | "agent" | "system"; actorId: string; status: "confirmed" | "rejected"; approvalRef: string; evidenceRef: string; decidedAt: string; leadTimeDays?: number; validUntil?: string },
) {
  ...
  if (decision.status === "confirmed" && (!decision.validUntil || Number.isNaN(Date.parse(decision.validUntil)) || Date.parse(decision.validUntil) <= Date.parse(decision.decidedAt))) throw new Error("A confirmed delivery requires a future validity deadline");
  return validateDeliveryConfirmation({
    ...pending,
    ...
    result: {
      ...
      ...(decision.status === "confirmed" ? { confirmed_lead_time_days: decision.leadTimeDays, valid_until: decision.validUntil } : {}),
    },
  });
}

export function expireDeliveryConfirmation(pending: DeliveryConfirmation, actorType: "system" | "agent" | "human") {
  if (pending.status !== "pending" && pending.status !== "confirmed") throw new Error("Only a pending or confirmed delivery confirmation can expire");
