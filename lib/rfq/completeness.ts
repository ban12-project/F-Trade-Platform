...
export function assessRfq(rfq: Rfq) {
  ...
  let score = 0;
  if (product.product_type) score += 10; else missing.push("product_type");
  if (product.oe_number || (product.vehicle_brand && product.vehicle_model)) score += 40; else missing.push("vehicle_model_or_oe_number");
  if (Number.isInteger(commercial.quantity) && commercial.quantity > 0) score += 25; else missing.push("quantity");
  if (typeof commercial.destination === "string" && commercial.destination.trim()) score += 25; else missing.push("destination");
...
export function updateRfqDraft(rfq: Rfq, patch: Rfq) {
  const next = { ...rfq, ...patch, product: { ...(rfq.product ?? {}), ...(patch.product ?? {}) }, commercial: { ...(rfq.commercial ?? {}), ...(patch.commercial ?? {}) } };
...
export function promoteRfqReady(rfq: Rfq) {
  ...
  if (!assessment.ready) throw new Error(`RFQ is incomplete: ${assessment.missing_fields.join(", ")}`);
