...
export async function saveSocialChannelControlAction(
  ...
  if (!session || session.user.role !== "admin") return { status: "error", message: "无权修改社交渠道控制状态。" };
  const parsed = socialControlChangeSchema.safeParse({
    channelRef: formData.get("channelRef"), accountRef: formData.get("accountRef"), action: formData.get("action"),
    actorType: "human", actorId: session.user.id, evidenceRef: formData.get("evidenceRef"),
  });
