...
export async function submitApprovedVideoJobAction(
  ...
  if (!session || !hasPermission(session.user.role, "video:write"))
    return { status: "error", message: "无权提交视频生成任务。" };
  try {
  ...
  const parsed = videoJobSubmissionSchema.safeParse({
    ...
    requiredCapabilities: formData
      .getAll("requiredCapabilities")
  ...
  if (!parsed.success)
    return { status: "error", message: parsed.error.issues[0]?.message ?? "视频生成请求无效。" };
  try {
    const job = await submitApprovedVideoJob(parsed.data, session.user.id);
    return {
      status: "success",
      message: "视频生成任务已排队，仍需受控 worker 执行。",
      jobId: job.id,
    };
  } catch (error) {
    return {
      status: "error",
      message: error instanceof Error ? error.message : "无法提交视频生成任务。",
    };
  }
}
