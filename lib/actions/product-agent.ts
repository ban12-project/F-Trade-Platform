...
export const initialProductAgentActionState: ProductAgentActionState = { status: "idle", message: "" };
...
export async function runProductAgentAction(_previous: ProductAgentActionState, formData: FormData): Promise<ProductAgentActionState> {
  ...
  try {
    ...
    const saved = await createProductAgentDraft(result.draft, session.user.id, { prompt_version: result.metadata.prompt_version, prompt_hash: result.metadata.prompt_hash });
    revalidatePath("/console/products");
    revalidatePath("/console/product-agent");
