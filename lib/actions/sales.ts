...
export async function createRfqAction(_previous: SalesActionState, formData: FormData): Promise<SalesActionState> {
  ...
  if (!session || !hasPermission(session.user.role, "sales:write")) return { status: "error", message: "无权录入询盘。" };
...
export async function submitRfqReadyAction(_previous: SalesActionState, formData: FormData): Promise<SalesActionState> {
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session || !hasPermission(session.user.role, "sales:write")) return { status: "error", message: "无权提交 RFQ。" };
  ...
  try {
    ...
    revalidatePath("/workspace");
    if (projectId) revalidatePath(`/workspace/${projectId}`);
    return result.state === "RFQ_READY" ? { status: "success", message: "RFQ 已 Ready，可由人工创建报价草稿。", rfqId } : { status: "error", message: `RFQ 仍缺少：${result.missingFields.join("、")}。` };
  } catch (error) { return { status: "error", message: error instanceof Error ? error.message : "无法提交 RFQ。" }; }
}

export async function reviseRfqAction(_previous: SalesActionState, formData: FormData): Promise<SalesActionState> {
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session || !hasPermission(session.user.role, "sales:write")) return { status: "error", message: "无权补充询盘。" };
  const rfqId = formData.get("rfqId");
  if (typeof rfqId !== "string" || !z.uuid().safeParse(rfqId).success) return { status: "error", message: "RFQ 标识无效。" };
  const parsed = rfqFormSchema.safeParse(Object.fromEntries(formData));
  if (!parsed.success) return { status: "error", message: parsed.error.issues[0]?.message ?? "询盘资料格式不正确。" };
  try {
    const projectId = projectIdFrom(formData);
    ...
    revalidatePath("/workspace");
    if (projectId) revalidatePath(`/workspace/${projectId}`);
    return { status: "success", message: result.draft.missing_fields.length ? `询盘已更新，还需补充：${result.draft.missing_fields.join("、")}。` : "询盘信息已完整，可以提交报价交接。", rfqId };
  } catch (error) { return { status: "error", message: error instanceof Error ? error.message : "无法更新询盘。" }; }
}
