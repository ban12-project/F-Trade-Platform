...
export async function createContentDraftAction(
  ...
  if (!session || !hasPermission(session.user.role, "content:write"))
    return { status: "error", message: "无权创建内容草稿。" };
  ...
  try {
    ...
    return {
      status: "success",
      message: `内容草稿已创建（${result.id.slice(0, 8)}），仍需 Gate 01 人工审核。`,
      contentId: result.id,
    };
  } catch (error) {
    return {
      status: "error",
      message: error instanceof Error ? error.message : "无法创建内容草稿。",
    };
  }
}
...
export async function decideContentReviewAction(
  ...
  if (!session || !hasPermission(session.user.role, "content:review"))
    return { status: "error", message: "无权执行 Gate 01 内容审核。" };
  ...
  try {
    ...
    return {
      status: "success",
      message:
        result.state === "CONTENT_APPROVED"
          ? "Gate 01 已批准，内容等待官方渠道发布。"
          : "Gate 01 已退回，内容需要修订。",
    };
  } catch (error) {
    return {
      status: "error",
      message: error instanceof Error ? error.message : "无法完成内容审核。",
    };
  }
}
...
export async function reviseContentDraftAction(
  ...
  if (!parsedContentId.success)
    return {
      status: "error",
      message: parsedContentId.error.issues[0]?.message ?? "内容记录标识无效。",
    };
  const parsed = contentDraftFormSchema.safeParse(Object.fromEntries(formData));
  if (!parsed.success)
    return { status: "error", message: parsed.error.issues[0]?.message ?? "内容资料格式不正确。" };
  try {
    ...
    return {
      status: "error",
      message: error instanceof Error ? error.message : "无法保存内容修订。",
    };
  }
}
...
export async function copyContentDraftToProjectAction(
  ...
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session || !hasPermission(session.user.role, "content:write"))
    return { status: "error", message: "无权复制内容草稿。" };
  const parsed = z
    .object({ projectId: z.uuid(), sourceContentId: z.uuid() })
  ...
  try {
    ...
    return {
      status: "error",
      message: error instanceof Error ? error.message : "无法复制内容草稿。",
    };
  }
}
