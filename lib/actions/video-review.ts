...
export async function decideVideoReviewAction(
  _previous: VideoReviewActionState,
  formData: FormData,
): Promise<VideoReviewActionState> {
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session || !hasPermission(session.user.role, "content:review"))
    return { status: "error", message: "无权确认视频计划。" };
  ...
  try {
    ...
    return {
      status: "error",
      message: error instanceof Error ? error.message : "无法确认视频计划。",
    };
  }
}
