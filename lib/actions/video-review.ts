...
export const initialVideoReviewActionState: VideoReviewActionState = { status: "idle", message: "" };
...
export async function decideVideoReviewAction(_previous: VideoReviewActionState, formData: FormData): Promise<VideoReviewActionState> {
  ...
  try {
    ...
    revalidatePath("/console"); revalidatePath("/console/video"); revalidatePath("/studio");
