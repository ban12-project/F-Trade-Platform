...
export async function createVideoProjectAction(
  ...
  if (!session || !hasPermission(session.user.role, "video:write"))
    return { status: "error", message: "无权创建视频项目。" };
  const parsed = videoProjectDraftFormSchema.safeParse({
  ...
  if (!parsed.success)
    return {
      status: "error",
      message: parsed.error.issues[0]?.message ?? "视频项目资料格式不正确。",
    };
  try {
    const files = formData
      .getAll("assets")
      .filter((value): value is File => value instanceof File && value.size > 0);
    const uploadedAssets = await prepareUploadedVideoAssets(
      files,
      session.user.id,
...
export async function createVideoProjectFromCanvasAction(
  document: VideoCanvasDocument,
): Promise<VideoActionState> {
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session || !hasPermission(session.user.role, "video:write"))
  ...
  try {
    ...
    return {
      status: "error",
      message: error instanceof Error ? error.message : "无法从画布创建视频项目。",
    };
  }
}
