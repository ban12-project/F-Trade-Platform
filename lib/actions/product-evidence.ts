...
export async function uploadProductEvidenceAction(_previous: ProductEvidenceActionState, formData: FormData): Promise<ProductEvidenceActionState> {
  ...
  if (!session || !hasPermission(session.user.role, "product:write")) return { status: "error", message: "无权上传产品证据。" };
  try {
    const projectId = z.uuid("项目标识无效。").parse(formData.get("projectId"));
    const document = formData.get("document");
    if (!(document instanceof File) || document.size === 0) throw new Error("请选择要上传的 PDF、CSV、XLS 或 XLSX 文件。");
  ...
  } catch (error) { return { status: "error", message: error instanceof Error ? error.message : "无法上传产品证据。" }; }
}
