...
import {
  type ContentPublicationPolicy,
  validatePublicationPolicy,
} from "../content/publication-policy";
import type { SocialChannelCircuitState } from "./circuit-breaker";

export const controlledPublicationRequestSchema = z
  .object({
...
export function createControlledPublicationCommand(
  ...
  if (policy.transport !== "camofox_controlled_mvp1")
    throw new Error("Controlled publication command requires the CamoFox MVP1 transport");
  if (circuit.status !== "active") throw new Error("Social channel is paused and cannot publish");
  if (circuit.channelRef !== policy.channelRef || circuit.accountRef !== policy.accountRef)
    throw new Error("Circuit state does not match publication policy");
  if (
    content.status !== "approved" ||
    typeof content.approval_ref !== "string" ||
    !content.approval_ref.trim()
  ) {
