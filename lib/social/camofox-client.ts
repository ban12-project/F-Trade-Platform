...
export const camofoxWorkerConfigSchema = z
  .object({
...
export class CamofoxWorkerClient {
  ...
  async healthcheck() {
    ...
    if (health.ok !== true || health.browserConnected !== true)
      throw new Error("CamoFox is not healthy and browser-connected");
    return {
  ...
  async createFacebookTab() {
    const created = await this.call("/tabs", {
  ...
  async snapshot(tabId: string) {
    const value = await this.call(
      `/tabs/${encodeURIComponent(tabId)}/snapshot?userId=${encodeURIComponent(this.config.userId)}&format=json&includeScreenshot=false`,
    );
    if (typeof value.snapshot !== "string")
      throw new Error("CamoFox did not return an accessibility snapshot");
