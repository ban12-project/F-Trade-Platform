...
export function applyWorkerObservation(
  state: SocialChannelCircuitState,
  observation: WorkerObservation,
): SocialChannelCircuitState {
  ...
  const reason =
    ...
    (observation.externalEffectCertain ? undefined : "external_result_unknown");
  if (!reason) return state;
  return {
    ...state,
    status: "paused",
    stopReason: socialStopReasonSchema.parse(reason),
    pausedAt: new Date(),
  };
}

/** A resumed channel requires an explicit human acknowledgement of the resolved stop reason. */
export function resumeSocialChannel(
  state: SocialChannelCircuitState,
  ...
  if (!acknowledgementRef.trim())
    throw new Error("Resuming a social channel requires an acknowledgement reference");
