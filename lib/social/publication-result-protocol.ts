...
export const signedPublicationResultBodySchema = z
  .object({
    result: z
      .object({
      ...
      .superRefine((value, context) => {
        if (value.outcome === "published" && !value.externalPublicationRef)
          context.addIssue({
            ...
            message: "Published result requires an external reference",
          });
...
export function verifyPublicationWorkerResult(
  bodyInput: unknown,
