...
function aad(scope: VaultScope, purpose: VaultPurpose, keyId: string) {
  ...
  return Buffer.from(JSON.stringify(["f-trade-facebook-vault-v1", scope.channelRef, scope.accountRef, purpose, keyId]));
}
 ...
 */
export function encryptFacebookCredential(value: unknown, scope: VaultScope, purpose: VaultPurpose, ring: VaultKeyring) {
  ...
  return ["fbv1", id, iv.toString("base64url"), cipher.getAuthTag().toString("base64url"), ciphertext.toString("base64url")].join(".");
}
export function decryptFacebookCredential(value: string, scope: VaultScope, purpose: VaultPurpose, ring: VaultKeyring): unknown {
  try {
    if (value.length > 24_000) throw bad();
    const [version, id, iv, tag, encrypted, extra] = value.split(".");
    if (extra !== undefined || version !== "fbv1" || !/^[A-Za-z0-9_-]{1,64}$/.test(id) ||
        !/^[A-Za-z0-9_-]{16}$/.test(iv) || !/^[A-Za-z0-9_-]{22}$/.test(tag) || !/^[A-Za-z0-9_-]+$/.test(encrypted)) throw bad();
    ...
    try { return JSON.parse(plain.toString("utf8")) as unknown; }
