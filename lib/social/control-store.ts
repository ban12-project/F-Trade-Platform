...
import { applySocialControlChange, socialControlChangeSchema, type SocialControlRecord } from "./control-record";
...
export async function saveSocialChannelControl(
  ...
  return database.transaction(async (tx) => {
    ...
    const now = new Date();
    const values = {
      id: existing?.id ?? randomUUID(), channelRef: parsed.channelRef, accountRef: parsed.accountRef,
      ...next, changedBy: parsed.actorId, changedAt: now,
    };
    const [saved] = await tx.insert(socialChannelControl).values(values).onConflictDoUpdate({
      target: [socialChannelControl.channelRef, socialChannelControl.accountRef], set: values,
    }).returning();
    await tx.insert(auditEvent).values({
      id: randomUUID(), action: `social_channel_${parsed.action}`, actorType: "human", actorId: parsed.actorId,
