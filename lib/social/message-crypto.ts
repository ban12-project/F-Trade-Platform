...
function messageEncryptionKey() {
  ...
  if (!encoded)
    throw new Error("SOCIAL_MESSAGE_ENCRYPTION_KEY is required to store social message bodies");
  const key = Buffer.from(encoded, "base64");
  if (key.length !== 32)
    throw new Error("SOCIAL_MESSAGE_ENCRYPTION_KEY must be a base64-encoded 32-byte key");
  return key;
}
...
export function decryptSocialMessageBody(value: string) {
  ...
  if (prefix !== PREFIX || !encodedIv || !encodedTag || !encodedCiphertext)
    throw new Error("Stored social message is invalid");
