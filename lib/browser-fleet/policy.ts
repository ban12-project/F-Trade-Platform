...
export type RunStatus = "queued" | "starting" | "running" | "stopping" | "quarantined" | "completed" | "failed" | "unknown";
...
export function initialState(limits: Limits): FleetState {
  return { version: 1, limits, installationId: null, bootId: null, capabilities: [], lastSeenAt: 0, accounts: [], runs: [] };
}
export function bindInstallation(state: FleetState, installationId: string) {
  if (state.installationId && state.installationId !== installationId) throw new Error("node_already_installed");
...
export function enqueueRun(state: FleetState, input: Pick<Run, "id" | "accountId" | "kind" | "jobRef" | "requestedBy" | "authSessionId">, now: number) {
  ...
  const duplicate = state.runs.find((r) => r.accountId === input.accountId && r.kind === input.kind && (
  ...
  if (input.kind !== "interactive" && account.authState !== "ready") throw new Error("account_needs_attention");
  const run: Run = { ...input, createdAt: now, status: "queued", claimId: null, leaseId: null, leaseUntil: 0, deadline: 0,
    credentialVersion: account.credentialVersion, stopRequested: false, failure: null,
    ticketHash: null, connectBefore: 0, ticketUsed: false };
  state.runs.push(run);
  return run;
}
export function sweep(state: FleetState, now: number) {
  for (const run of state.runs) {
    if (run.status === "queued" && run.kind === "interactive" && now - run.createdAt >= 900_000) {
      run.status = "failed"; run.failure = "interactive_queue_expired";
    }
  ...
  const terminal = state.runs.filter((r) => r.kind !== "publish" && !isLive(r) && r.status !== "queued");
...
export function scheduleInbox(state: FleetState, now: number, newId: () => string) {
  ...
  for (const a of state.accounts) {
    if (!a.enabled || a.authState !== "ready" || a.pollSeconds === 0 || a.nextPollAt > now) continue;
    if (state.runs.some((r) => r.accountId === a.id && r.kind === "inbox" && (r.status === "queued" || isLive(r)))) continue;
    if (state.runs.filter((r) => r.status === "queued").length >= 200) break;
    enqueueRun(state, { id: newId(), accountId: a.id, kind: "inbox", jobRef: null, requestedBy: "scheduler", authSessionId: null }, now);
  }
}
export function claimRun(state: FleetState, input: { requestId: string; leaseId: string; availableMemoryMb: number; localSlots: number }, now: number) {
  sweep(state, now);
  const previous = state.runs.find((r) => r.claimId === input.requestId);
  if (previous) return isLive(previous) && previous.status !== "quarantined" && !previous.stopRequested ? previous : null;
  ...
  const candidates = state.runs.filter((r) => {
    ...
    return r.status === "queued" && !r.stopRequested && !occupied.has(r.accountId) && a?.enabled &&
      state.capabilities.includes(r.kind) && (r.kind === "interactive" || a.authState === "ready");
...
export function renewRun(state: FleetState, runId: string, leaseId: string, ready: boolean, now: number) {
  const run = state.runs.find((r) => r.id === runId);
  const a = state.accounts.find((a) => a.id === run?.accountId);
  if (!run || !a?.enabled || run.leaseId !== leaseId || !isLive(run) || run.status === "quarantined" ||
    run.stopRequested || run.leaseUntil <= now || run.deadline <= now || run.credentialVersion !== a.credentialVersion) return null;
  run.leaseUntil = Math.min(now + LEASE_MS, run.deadline);
...
export function requestStop(state: FleetState, run: Run) {
  run.stopRequested = true;
  run.ticketHash = null;
  if (run.status === "queued") { run.status = "failed"; run.failure = "cancelled"; }
  else if (isLive(run)) run.status = "stopping";
}
export function finishRun(state: FleetState, runId: string, outcome: "completed" | "failed" | "needs_login" | "needs_2fa" | "checkpoint" | "unknown", stopped: boolean, now: number) {
  const run = state.runs.find((r) => r.id === runId);
  if (!run || !isLive(run)) return;
  if (!stopped) { requestStop(state, run); return; }
  const a = state.accounts.find((a) => a.id === run.accountId);
  run.ticketHash = null;
  ...
  run.status = run.kind === "publish" ? "unknown" : outcome === "completed" ? "completed" : "failed";
  run.failure = run.status === "completed" ? null : outcome;
  if (a) {
    if (["needs_login", "needs_2fa", "checkpoint"].includes(outcome)) a.authState = outcome as AuthState;
...
export function publicState(state: FleetState) {
  return { limits: state.limits, lastSeenAt: state.lastSeenAt, capabilities: state.capabilities,
    accounts: state.accounts.map(({ loginCiphertext, proxyCiphertext, ...a }) => ({ ...a, loginSaved: !!loginCiphertext, proxySaved: !!proxyCiphertext })),
