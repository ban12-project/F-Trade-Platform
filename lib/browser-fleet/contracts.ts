...
}).strict().refine((v) => v.memoryBudgetMb >= v.browserMemoryMb, { path: ["memoryBudgetMb"], message: "内存预算至少容纳一个浏览器。" });
export const accountFormSchema = z.object({
  nodeId: id,
  channelRef: reference,
  accountRef: reference,
  pollSeconds: z.number().int().min(0).max(86400).refine((v) => v === 0 || v >= 300),
...
export const ownerCommandSchema = z.discriminatedUnion("operation", [
  ...
  z.object({ operation: z.literal("account"), nodeId: id, accountId: id, enabled: z.boolean() }).strict(),
  z.object({ operation: z.literal("confirm-login"), nodeId: id, accountId: id, confirmed: z.literal(true) }).strict(),
...
export const nodeRequestSchema = z.discriminatedUnion("operation", [
  ...
  z.object({ ...common, operation: z.literal("recover"), stoppedRunIds: z.array(id).max(100), capabilities: z.array(z.enum(["interactive", "inbox", "publish"])).max(3) }).strict(),
  z.object({ ...common, operation: z.literal("claim"), requestId: id, availableMemoryMb: z.number().int().min(0).max(1048576), localSlots: z.number().int().min(0).max(16) }).strict(),
  z.object({ ...common, operation: z.literal("heartbeat"), runId: id, leaseId: id, ready: z.boolean() }).strict(),
  z.object({ ...common, operation: z.literal("finish"), runId: id, leaseId: id, stopped: z.literal(true), outcome: z.enum(["completed", "failed", "needs_login", "needs_2fa", "checkpoint", "unknown"]) }).strict(),
  z.object({ ...common, operation: z.literal("admit"), ticket: z.string().min(70).max(128) }).strict(),
]);
