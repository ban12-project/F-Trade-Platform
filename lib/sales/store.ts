...
export type RfqEntry = {
  ...
  formValues: Omit<RfqFormInput, "evidenceRef">;
};

function rfqFromInput(input: RfqFormInput, id: string) {
  const product = {
    product_type: input.productType,
    ...(input.oeNumber ? { oe_number: input.oeNumber } : {}),
    ...(input.vehicleBrand ? { vehicle_brand: input.vehicleBrand } : {}),
    ...(input.vehicleModel ? { vehicle_model: input.vehicleModel } : {}),
  };
  const commercial = {
    ...(input.quantity ? { quantity: Number(input.quantity) } : {}),
    ...(input.destination ? { destination: input.destination } : {}),
  };
  return updateRfqDraft(
    {
      rfq_id: id,
      ...(input.leadId ? { lead_ref: input.leadId } : {}),
      customer: {
        ...(input.customerName ? { name: input.customerName } : {}),
        ...(input.customerCompany ? { company: input.customerCompany } : {}),
...
function entryFromRecord(row: typeof aggregateRecord.$inferSelect): RfqEntry | null {
  ...
  if (
    ...
    !productTypes.has(product.product_type as RfqFormInput["productType"])
  )
    return null;
  ...
  return {
    ...
    missingFields: Array.isArray(payload.missing_fields)
      ? payload.missing_fields.filter((field): field is string => typeof field === "string")
      : [],
    completenessScore:
      typeof payload.completeness_score === "number" ? payload.completeness_score : 0,
    formValues: {
      leadId: typeof payload.lead_ref === "string" ? payload.lead_ref : "",
      customerName: typeof customer?.name === "string" ? customer.name : "",
      customerCompany: typeof customer?.company === "string" ? customer.company : "",
      customerCountry: typeof customer?.country === "string" ? customer.country : "",
      productType: product.product_type as RfqFormInput["productType"],
      oeNumber: typeof product.oe_number === "string" ? product.oe_number : "",
      vehicleBrand: typeof product.vehicle_brand === "string" ? product.vehicle_brand : "",
      vehicleModel: typeof product.vehicle_model === "string" ? product.vehicle_model : "",
...
const productTypes = new Set<RfqFormInput["productType"]>([
  "clutch_disc",
  "clutch_cover",
  "release_bearing",
  "clutch_kit",
        ...
        const [lead] = await tx
          ...
          .where(
            and(
              ...
              eq(workspaceProjectItem.aggregateId, input.leadId),
...
export async function listRfqEntries(limit = 50): Promise<RfqEntry[]> {
  const rows = await getDatabase()
    ...
    .where(eq(aggregateRecord.type, "rfq"))
...
export async function reviseRfq(
  ...
  return getDatabase().transaction(async (tx) => {
    if (projectId) {
      const [link] = await tx
        ...
        .where(
          and(
            eq(workspaceProjectItem.projectId, projectId),
            eq(workspaceProjectItem.aggregateId, rfqId),
          ),
        );
    ...
    const [updated] = await tx
      ...
      .set({ payload: draft, version: sql`${aggregateRecord.version} + 1` })
...
export async function submitRfqReady(
  rfqId: string,
  evidenceRef: string,
  actorId: string,
  projectId?: string,
  ...
  return getDatabase().transaction(async (tx) => {
    ...
    const [updated] = await tx
      .update(aggregateRecord)
      .set({ state: "RFQ_READY", payload: ready, version: sql`${aggregateRecord.version} + 1` })
      .where(and(eq(aggregateRecord.id, rfqId), eq(aggregateRecord.version, record.version)))
      .returning({ id: aggregateRecord.id });
    if (!updated) throw new Error("RFQ 已被其他操作更新，请刷新后重试。");
