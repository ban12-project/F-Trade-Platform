...
export function applyInquiryMessage(rfq: Rfq, message: string) {
  ...
  const destination = message.match(/\b(?:to\s+|destination\s*[:=-]?\s*)([A-Za-z][A-Za-z -]{2,})\b/i);
  if (destination) patch.commercial.destination = destination[1].trim();
  const vehicle = message.match(/\b(Toyota|Honda|Ford|Volkswagen)\s+([A-Za-z0-9 -]{2,}?)(?:\s+clutch|,|\.|$)/i);
  if (vehicle) {
...
export function nextClarification(rfq: Rfq) {
  ...
  if (!first) return { status: "ready" as const, message: "RFQ is complete and ready for human quotation handoff." };
  const prompts: Record<string, string> = {
    product_type: "Which clutch component do you need?",
    vehicle_model_or_oe_number: "Please provide the OE number or the exact vehicle brand and model.",
    quantity: "What quantity do you need?",
    destination: "What is the destination country or port?",
  };
  return { status: "clarification_required" as const, field: first, message: prompts[first] ?? "Please provide the missing RFQ detail." };
}

export function assertNoAgentQuotation(rfq: Rfq) {
  if (rfq.missing_fields.length) throw new Error("Agent must continue clarification and cannot quote an incomplete RFQ");
