...
function splitHashtags(value: string) {
  return [...new Set(value.split(/[\s,，;；]+/).map((item) => item.trim()).filter(Boolean).map((item) => item.startsWith("#") ? item : `#${item}`))];
}
...
export function buildContentDraft(input: ContentDraftInput, readyProduct: ProductReady, contentId: string = randomUUID()) {
  if (readyProduct.verification_status !== "verified" || readyProduct.record_id !== input.productId) {
  ...
  const productNameFact = presentFactOptions(readyProduct).find((fact) => fact.path === "product.product_name");
  ...
  return parseContent({
    ...
    hook: input.hook,
    body: input.body,
    product_facts: sourceFact.path === productNameFact.path
      ? [{ field: sourceFact.path, value: sourceFact.value, evidence_ref: sourceFact.evidenceRef }]
      : [
        { field: productNameFact.path, value: productNameFact.value, evidence_ref: productNameFact.evidenceRef },
        { field: sourceFact.path, value: sourceFact.value, evidence_ref: sourceFact.evidenceRef },
      ],
...
function contentEntry(
  ...
  const productName = content.product_facts?.find((fact) => fact.field === "product.product_name")?.value ?? "已核验产品";
  return {
    id: row.id,
    state: row.state,
...
export async function listReadyProductContentSources(): Promise<ReadyProductContentSource[]> {
  ...
  return rows.flatMap((row) => {
    ...
    return [{ id: row.id, productName: product.product.product_name, internalSku: product.product.internal_sku, factOptions: presentFactOptions(product) }];
  });
}
...
async function latestApprovals(rows: Array<typeof aggregateRecord.$inferSelect>) {
  if (rows.length === 0) return new Map<string, { id: string; status: "pending" | "approved" | "rejected" }>();
  const approvalRows = await getDatabase().select({ id: approval.id, aggregateId: approval.aggregateId, status: approval.status }).from(approval)
  ...
  for (const row of approvalRows) if (!results.has(row.aggregateId)) results.set(row.aggregateId, row);
  return results;
}
...
export async function getContentCatalogDetail(contentId: string): Promise<ContentCatalogDetail | null> {
  ...
  const [row] = await database.select().from(aggregateRecord).where(and(eq(aggregateRecord.id, contentId), eq(aggregateRecord.type, "content"))).limit(1);
  if (!row) return null;
  const [approvalRow] = await database.select({ id: approval.id, status: approval.status }).from(approval)
...
export async function decideContentReview(input: ContentReviewInput, actorId: string) {
  ...
  const eventId = randomUUID();
  return getDatabase().transaction(async (tx) => {
    const [aggregate] = await tx.select({ id: aggregateRecord.id, state: aggregateRecord.state, version: aggregateRecord.version, payload: aggregateRecord.payload }).from(aggregateRecord)
      .where(and(eq(aggregateRecord.id, input.contentId), eq(aggregateRecord.type, "content"))).for("update");
    if (!aggregate || aggregate.state !== "CONTENT_REVIEW_REQUIRED") throw new Error("该内容当前不处于待审核状态。");
    ...
    const nextState = input.decision === "approved" ? "CONTENT_APPROVED" : "CONTENT_REVISION_REQUIRED";
    const content = parseContent({ ...aggregate.payload as ContentRecord, status: input.decision === "approved" ? "approved" : "revision_required", ...(input.decision === "approved" ? { approval_ref: pendingApproval.id } : {}) });
    assertTransition({ eventId, entityType: "content", entityId: aggregate.id, fromState: "CONTENT_REVIEW_REQUIRED", toState: nextState, actorType: "human", actorId, occurredAt: now.toISOString(), evidenceRefs: [input.evidenceRef], gate: "gate_01_truth", approvalRef: pendingApproval.id }, { id: pendingApproval.id, aggregateId: aggregate.id, gate: "gate_01_truth", status: input.decision, decidedByType: "human", decidedById: actorId, evidenceRef: input.evidenceRef });
    ...
    const [updated] = await tx.update(aggregateRecord).set({ state: nextState, payload: content, version: sql`${aggregateRecord.version} + 1` }).where(and(eq(aggregateRecord.id, aggregate.id), eq(aggregateRecord.version, aggregate.version))).returning({ id: aggregateRecord.id });
    if (!updated) throw new Error("内容审核与另一项操作冲突，请刷新后重试。");
    await tx.insert(workflowEvent).values({ id: eventId, aggregateId: aggregate.id, fromState: "CONTENT_REVIEW_REQUIRED", toState: nextState, actorType: "human", actorId, gate: "gate_01_truth", approvalId: pendingApproval.id, evidenceRefs: [input.evidenceRef], occurredAt: now });
    await tx.insert(auditEvent).values({ id: randomUUID(), action: "content_gate_01_decided", actorType: "human", actorId, aggregateId: aggregate.id, subjectType: "content", subjectId: aggregate.id, metadata: { decision: input.decision, approval_id: pendingApproval.id }, occurredAt: now });
    return { state: nextState, approvalId: pendingApproval.id };
  });
}

export async function reviseContentDraft(contentId: string, input: ContentDraftInput, actorId: string) {
  const now = new Date();
  const eventId = randomUUID();
  const approvalId = randomUUID();
  return getDatabase().transaction(async (tx) => {
    const [aggregate] = await tx.select({ id: aggregateRecord.id, state: aggregateRecord.state, version: aggregateRecord.version, payload: aggregateRecord.payload }).from(aggregateRecord)
      .where(and(eq(aggregateRecord.id, contentId), eq(aggregateRecord.type, "content"))).for("update");
    if (!aggregate || aggregate.state !== "CONTENT_REVISION_REQUIRED") throw new Error("该内容当前不处于待修订状态。");
    const currentContent = aggregate.payload as ContentRecord;
    if (input.productId !== currentContent.product_id) throw new Error("修订不能改写内容所引用的产品。请另建内容草稿。");
    const [product] = await tx.select({ state: aggregateRecord.state, payload: aggregateRecord.payload }).from(aggregateRecord)
      .where(and(eq(aggregateRecord.id, input.productId), eq(aggregateRecord.type, "product"))).for("update");
    if (!product || product.state !== "PRODUCT_READY") throw new Error("引用产品不再处于 Product Ready，不能重新送审。");
    const content = buildContentDraft(input, product.payload as unknown as ProductReady, contentId);
    assertTransition({ eventId, entityType: "content", entityId: contentId, fromState: "CONTENT_REVISION_REQUIRED", toState: "CONTENT_REVIEW_REQUIRED", actorType: "human", actorId, occurredAt: now.toISOString(), evidenceRefs: content.product_facts.map((fact) => fact.evidence_ref) });
    const [updated] = await tx.update(aggregateRecord).set({ state: "CONTENT_REVIEW_REQUIRED", payload: content, version: sql`${aggregateRecord.version} + 1` })
      .where(and(eq(aggregateRecord.id, aggregate.id), eq(aggregateRecord.version, aggregate.version))).returning({ id: aggregateRecord.id });
    if (!updated) throw new Error("内容修订与另一项操作冲突，请刷新后重试。");
    await tx.insert(approval).values({ id: approvalId, aggregateId: contentId, gate: "gate_01_truth", status: "pending", requestedByType: "human", requestedById: actorId, requestedAt: now });
    await tx.insert(workflowEvent).values({ id: eventId, aggregateId: contentId, fromState: "CONTENT_REVISION_REQUIRED", toState: "CONTENT_REVIEW_REQUIRED", actorType: "human", actorId, evidenceRefs: content.product_facts.map((fact) => fact.evidence_ref), occurredAt: now });
    await tx.insert(auditEvent).values({ id: randomUUID(), action: "content_draft_revised", actorType: "human", actorId, aggregateId: contentId, subjectType: "content", subjectId: contentId, metadata: { approval_id: approvalId, content_type: content.content_type }, occurredAt: now });
