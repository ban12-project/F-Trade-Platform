...
export function validatePublicationPolicy(policy: ContentPublicationPolicy) {
  ...
  if (policy.officialApi !== true) throw new Error("Content publication requires an official API channel");
  if (!policy.publishingEnabled) throw new Error("Content publication is not enabled for this channel");
  return policy;
}

export function publishThroughOfficialChannel(
  ...
  if (!publicationRef.trim()) throw new Error("Official publication requires an external publication reference");
