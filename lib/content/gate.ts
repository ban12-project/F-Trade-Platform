export function decideContent(
  ...
  if (content.status !== "review_required")
    throw new Error("Content must be reviewed before a Gate 01 decision");
  if (decision.actorType !== "human")
    throw new Error("Gate 01 content decisions require a human actor");
  if (!decision.approvalRef.trim() || !decision.evidenceRef.trim())
    throw new Error("Gate 01 content decisions require approval and evidence");
  return {
    ...content,
