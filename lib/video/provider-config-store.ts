...
export const saveVideoProviderModelSettingsSchema = z.object({
  ...
  runtimeSettings: z.record(z.string().trim().min(1).max(120), z.string().trim().max(2_000)).default({}),
  model: videoModelCapabilitySchema,
  actorId: z.string().trim().min(1).max(240),
}).strict().superRefine((input, context) => {
  if (input.model.provider !== input.provider) {
    context.addIssue({ code: "custom", path: ["model", "provider"], message: "模型必须属于当前视频提供商。" });
  }
...
export type SaveVideoProviderModelSettingsInput = z.infer<typeof saveVideoProviderModelSettingsSchema>;
...
export async function listVideoProviderSettings(database: Database = getDatabase()): Promise<VideoProviderSettingsSummary[]> {
...
export async function loadVideoExecutionConfiguration(database: Database = getDatabase()): Promise<{
  ...
  return {
    ...
    policies: providers.map((provider) => ({
      provider: videoProviderIdSchema.parse(provider.provider),
      enabled: provider.enabled,
      credentialRef: provider.credentialCiphertext ? providerCredentialRef(videoProviderIdSchema.parse(provider.provider)) : null,
      maximumConcurrentJobs: provider.maximumConcurrentJobs,
      maximumAttempts: provider.maximumAttempts,
      budgetLimitCents: provider.budgetLimitCents,
      budgetCommittedCents: provider.budgetCommittedCents,
...
export async function resolveVideoProviderCredential(credentialRef: string, database: Database = getDatabase()): Promise<string> {
  const provider = providerFromCredentialRef(credentialRef);
  if (!provider) throw new Error("视频提供商凭据引用无效。");
  const [row] = await database.select().from(videoProviderConfig).where(and(
    eq(videoProviderConfig.provider, provider),
    eq(videoProviderConfig.enabled, true),
  ));
  if (!row?.credentialCiphertext) throw new Error("视频提供商未启用或未配置凭据。");
  return decryptStoredSecret(row.credentialCiphertext, "Stored video provider credential is invalid");
}
...
export async function saveVideoProviderModelSettings(
  ...
  await database.transaction(async (tx) => {
    ...
    await tx.insert(videoModelConfig).values(modelValues).onConflictDoUpdate({
