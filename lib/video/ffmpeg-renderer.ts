...
function assDocument(request: VideoRenderRequest) {
  const captions = request.timeline.scenes.flatMap((scene) => scene.subtitles).map((subtitle) =>
...
async function probeSource(filePath: string, ffprobeBin: string) {
  const { stdout } = await execFileAsync(ffprobeBin, ["-v", "error", "-show_entries", "format=duration:stream=codec_type", "-of", "json", filePath], { maxBuffer: 1024 * 1024 });
  ...
  return { durationSeconds: Number.isFinite(durationSeconds) ? durationSeconds : null, hasAudio: value.streams?.some((stream) => stream.codec_type === "audio") ?? false };
}
    ...
    async render(request: VideoRenderRequest) {
      ...
      try {
        ...
        for (const [index, scene] of request.timeline.scenes.entries()) {
          ...
          const sourceInfo = mediaType === "video" ? await probeSource(source, ffprobeBin) : { durationSeconds: null, hasAudio: false };
          if (sourceInfo.durationSeconds !== null && trimStartSeconds + scene.durationSeconds > sourceInfo.durationSeconds + 0.05) {
            throw new Error(`片段 ${scene.sceneId} 的截取区间超过源素材长度。`);
          }
          const inputArgs = mediaType === "image"
            ? ["-loop", "1", "-i", source]
          ...
          const audioArgs = keepSourceAudio
            ? ["-map", "0:v:0", "-map", "0:a:0"]
            : ["-f", "lavfi", "-t", String(scene.durationSeconds), "-i", "anullsrc=channel_layout=stereo:sample_rate=48000", "-map", "0:v:0", "-map", "1:a:0"];
          const framing = scene.fitMode === "cover"
          ...
          await execFileAsync(ffmpegBin, [
            "-hide_banner", "-loglevel", "error", "-y", ...inputArgs, ...audioArgs,
            "-t", String(scene.durationSeconds),
            "-vf", `${framing},fps=${request.fps}`,
