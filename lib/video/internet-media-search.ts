...
export const internetMediaSearchInputSchema = z.object({
  projectId: z.uuid("项目标识无效。"),
  productId: z.uuid("产品记录标识无效。"),
  query: z.string().trim().min(2, "检索词至少需要两个字符。").max(120, "检索词不能超过 120 个字符。"),
}).strict();

export const internetMediaResultIdSchema = z.string().regex(/^wikimedia:\d+$/, "互联网素材标识无效。");

export const internetMediaImportInputSchema = internetMediaSearchInputSchema.extend({
  resultIds: z.array(internetMediaResultIdSchema)
    .min(1, "至少选择一个互联网素材。")
    .max(3, "每条视频最多选择三个互联网素材。")
    .refine((ids) => new Set(ids).size === ids.length, "互联网素材不能重复选择。"),
}).strict();

export const internetMediaSearchResultSchema = z.object({
...
function cleanTitle(value: string) {
  return value.replace(/^File:/, "").replace(/_/g, " ").trim();
}

function parseCommonsPages(input: unknown): Array<InternetMediaSearchResult & { importUrl: string }> {
  ...
  return (response.query?.pages ?? []).flatMap((page) => {
    ...
    if (!page.pageid || !page.title || !image?.descriptionurl || !thumbnailUrl || !mimeType || !width || !height || !image.mime || !allowedMimeTypes.has(image.mime) || !allowedMimeTypes.has(mimeType)) return [];
    let mediaUrl: URL;
    ...
    if (mediaUrl.protocol !== "https:" || !allowedMediaHosts.has(mediaUrl.hostname) || sourcePageUrl.protocol !== "https:" || sourcePageUrl.hostname !== "commons.wikimedia.org") return [];
    return [{
      ...
      license: image.extmetadata?.LicenseShortName?.value?.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim().slice(0, 160) || "See source page",
...
async function fetchCommonsPages(url: URL, fetcher: InternetMediaFetcher) {
  const response = await fetcher(url, {
    headers: { Accept: "application/json", "User-Agent": "F-Trade/0.1 (private test media search)" },
...
export async function searchInternetVideoMedia(queryInput: string, fetcher: InternetMediaFetcher = fetch) {
  const query = internetMediaSearchInputSchema.shape.query.parse(queryInput);
  let pages = await fetchCommonsPages(commonsRequestUrl({ query: `intitle:"${query.replace(/["\\]/g, " ")}"`, thumbnailWidth: 960 }), fetcher);
  if (!pages.length) pages = await fetchCommonsPages(commonsRequestUrl({ query, thumbnailWidth: 960 }), fetcher);
  return pages.map(({ importUrl: _importUrl, ...result }) => internetMediaSearchResultSchema.parse(result));
}

async function resolveInternetMedia(resultIds: string[], fetcher: InternetMediaFetcher) {
  const ids = resultIds.map((id) => Number(internetMediaResultIdSchema.parse(id).slice("wikimedia:".length)));
  const pages = await fetchCommonsPages(commonsRequestUrl({ pageIds: ids, thumbnailWidth: 1920 }), fetcher);
...
function matchesImageSignature(mimeType: InternetMediaSearchResult["mimeType"], bytes: Uint8Array) {
  ...
  return new TextDecoder().decode(bytes.slice(0, 12)).startsWith("RIFF") && new TextDecoder().decode(bytes.slice(8, 12)) === "WEBP";
}
...
async function downloadInternetImage(result: InternetMediaSearchResult & { importUrl: string }, fetcher: InternetMediaFetcher) {
  const url = new URL(result.importUrl);
  if (url.protocol !== "https:" || !allowedMediaHosts.has(url.hostname)) throw new Error("互联网素材地址不在允许范围内。");
  const response = await fetcher(url, {
    headers: { Accept: result.mimeType, "User-Agent": "F-Trade/0.1 (private test media import)" },
    redirect: "manual",
    signal: AbortSignal.timeout(15_000),
  });
  if (response.status >= 300 && response.status < 400) throw new Error("互联网素材发生了未经允许的跳转。");
  ...
  if (!matchesImageSignature(result.mimeType, data)) throw new Error("互联网素材内容与声明格式不一致。");
  return new File([data], `internet-${result.id.replace(":", "-")}${extensionForMimeType(result.mimeType)}`, { type: result.mimeType });
}

/** Re-resolves opaque provider IDs and downloads only the provider-owned image URLs. */
export async function resolveInternetVideoMediaFiles(resultIdsInput: unknown, fetcher: InternetMediaFetcher = fetch) {
  const resultIds = z.array(internetMediaResultIdSchema).min(1).max(3)
    .refine((ids) => new Set(ids).size === ids.length, "互联网素材不能重复选择。")
...
async function storeProvenanceEvidence(
  ...
  const manifest = JSON.stringify({
    ...
    sources: results.map(({ id, title, sourcePageUrl, thumbnailUrl, mimeType, width, height, license }) => ({ id, title, sourcePageUrl, importedMediaUrl: thumbnailUrl, mimeType, width, height, license })),
  }, null, 2);
...
export async function importInternetVideoMedia(
  ...
  const { results: resolved, files } = await resolveInternetVideoMediaFiles(value.resultIds, fetcher);
  const provenanceEvidenceRef = await storeProvenanceEvidence(value.query, resolved, actorId, database);
