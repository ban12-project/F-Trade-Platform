...
export async function loadVideoCanvasDocument(
  ownerId: string,
  database: Database = getDatabase(),
): Promise<StoredVideoCanvasDocument | null> {
  const [row] = await database
    .select({ document: videoCanvasDocument.document, revision: videoCanvasDocument.revision })
...
export async function saveVideoCanvasDocument(
  input: SaveVideoCanvasInput,
  ownerId: string,
  database: Database = getDatabase(),
): Promise<StoredVideoCanvasDocument> {
  ...
  return database.transaction(async (tx) => {
    if (parsed.document.factBinding) {
      const [product] = await tx
        ...
        .where(
          and(
            eq(aggregateRecord.id, parsed.document.factBinding.productId),
    ...
    if (parsed.expectedRevision === 0) {
      const [created] = await tx
        ...
        .onConflictDoNothing({ target: videoCanvasDocument.ownerId })
        .returning({ revision: videoCanvasDocument.revision });
      if (created) {
        await tx.insert(auditEvent).values({
          id: randomUUID(),
          action: "video_canvas.created",
          actorType: "human",
          actorId: ownerId,
          subjectType: "video_canvas",
          subjectId: ownerId,
          metadata: {
            node_count: parsed.document.nodes.length,
            edge_count: parsed.document.edges.length,
            fact_binding: Boolean(parsed.document.factBinding),
          },
          occurredAt: now,
        });
    ...
    const [existing] = await tx
      .select({ revision: videoCanvasDocument.revision })
      .from(videoCanvasDocument)
      .where(eq(videoCanvasDocument.ownerId, ownerId))
      .for("update");
    ...
    await tx
      .update(videoCanvasDocument)
      .set({ document: parsed.document, revision, updatedAt: now })
      .where(eq(videoCanvasDocument.ownerId, ownerId));
    await tx.insert(auditEvent).values({
      id: randomUUID(),
      action: "video_canvas.saved",
      actorType: "human",
      actorId: ownerId,
      subjectType: "video_canvas",
      subjectId: ownerId,
