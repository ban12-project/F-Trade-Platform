...
const opaqueRef = (prefix: string) => z.string().trim().regex(new RegExp(`^${prefix}-[a-z0-9][a-z0-9_-]{2,120}$`, "i"));
...
export async function publishSocialVideoDraft(
  ...
  const policy = socialVideoPublicationPolicySchema.parse(policyInput);
  if (!/^evidence-[a-z0-9][a-z0-9_-]{2,120}$/i.test(humanConfirmationRef)) throw new Error("正式发布必须有人工确认的证据引用。 ");
  if (!policy.publishingEnabled) throw new Error("此渠道未启用正式发布。 ");
  if (draft.status !== "draft" || draft.channel !== policy.channel || draft.accountRef !== policy.accountRef) throw new Error("草稿与发布策略不匹配。 ");
  const credential = await resolveCredential(policy.credentialRef);
  if (!credential.trim()) throw new Error("社交渠道凭据不可用。 ");
