...
export const videoPresignedUploadPayloadSchema = z
  .object({
    receiptId: z.uuid(),
    projectId: z.uuid(),
...
export const completedVideoUploadTokenSchema = videoPresignedUploadPayloadSchema
  .extend({
    actorId: z.string().trim().min(1).max(160),
    blobPath: z.string().trim().min(1).max(1_024),
  })
  .strict()
  .refine((value) => value.blobPath === videoUploadBlobPath(value), {
    path: ["blobPath"],
    message: "上传回调路径与签名载荷不一致。",
  });

export const claimVideoUploadReceiptsSchema = z
  .array(z.uuid())
  .min(1)
  .max(3)
...
export function videoUploadBlobPath(
  input: Pick<VideoPresignedUploadPayload, "receiptId" | "projectId" | "contentType">,
) {
