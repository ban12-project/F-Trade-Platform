...
import { type MarketingVideoDraft, marketingVideoDraftSchema } from "./edit-contracts";
...
export const marketingTimelineInputSchema = z
  .object({
 ...
 */
export function createMarketingTimeline(
  projectInput: VideoProject,
  input: z.input<typeof marketingTimelineInputSchema>,
): MarketingTimeline {
  ...
  const scenes = project.scenes.map((scene) => {
    ...
    const timedNarration = narration
      ? { ...narration, startSeconds: sceneStart, durationSeconds: scene.durationSeconds }
      : undefined;
    return {
      ...
      ...(timedNarration
        ? {
            ...
            subtitles: [
              {
                text: timedNarration.text,
                startSeconds: sceneStart,
                endSeconds: startSeconds,
                claimRefs: timedNarration.claimRefs,
              },
            ],
          }
        : { subtitles: [] }),
  ...
  return {
    ...
    ...(value.soundtrackAssetRef ? { soundtrackAssetRef: value.soundtrackAssetRef } : {}),
    scenes,
  };
}

/** Builds the deterministic MVP1 edit timeline before any external media work begins. */
export function createMarketingEditTimeline(
  projectInput: VideoProject,
  draftInput: MarketingVideoDraft,
): MarketingTimeline {
  const project = videoProjectSchema.parse(projectInput);
  ...
  const scenes = draft.clips.map((clip) => {
    ...
    if (!source || source.mediaType !== clip.mediaType)
      throw new Error(`片段 ${clip.clipId} 引用了未授权或类型不匹配的素材。`);
    const verifiedClaim =
      clip.caption.kind === "verified_fact" ? claimsByField.get(clip.caption.claimRef) : undefined;
    if (clip.caption.kind === "verified_fact" && !verifiedClaim)
    ...
    return {
      ...
      subtitles: caption
        ? [{ ...caption, startSeconds: sceneStart, endSeconds: startSeconds }]
        : [],
    };
  });
  const cta = draft.ctaText
    ? { text: draft.ctaText, startSeconds: Math.max(0, startSeconds - 2), endSeconds: startSeconds }
    : undefined;
