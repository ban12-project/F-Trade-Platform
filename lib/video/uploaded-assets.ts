...
const allowedFiles: ReadonlyMap<
  string,
...
export function validateUploadedVideoSourceAsset(file: File) {
  ...
  if (!found.extensions.includes(extname(file.name).toLowerCase()))
    throw new Error("营销素材的文件扩展名必须与其媒体类型匹配。 ");
  if (file.size < 1 || file.size > maximumAssetBytes)
    throw new Error("单个营销素材必须介于 1 字节和 20MB 之间。 ");
  return found;
}

/** Stores user-provided visual assets privately and exposes only the existing evidence reference. */
export async function prepareUploadedVideoAssets(
  files: File[],
  actorId: string,
  ...
  if (files.reduce((total, file) => total + file.size, 0) > maximumTotalAssetBytes)
    throw new Error("一次上传的营销素材总计不能超过 20MB。 ");
  if (files.length && !rightsEvidenceRef.trim())
    throw new Error("上传营销素材时必须提供权利证据引用。 ");
...
export async function withTemporaryUploadedVideoAssets<T>(
  assetRefs: string[],
  callback: (paths: ReadonlyMap<string, string>) => Promise<T>,
): Promise<T> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const filePath = join(
        ...
        `${row.id}${extensionForContentType(row.contentType)}`,
      );
      await pipeline(
        Readable.fromWeb(asset.body as unknown as import("node:stream/web").ReadableStream),
