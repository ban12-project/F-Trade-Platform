...
function extensionForContentType(contentType: string) {
  return ({ "image/jpeg": ".jpg", "image/png": ".png", "image/webp": ".webp", "video/mp4": ".mp4", "video/quicktime": ".mov" } as Record<string, string>)[contentType];
}
...
export async function issueSandboxVideoSources(assetRefs: string[]) {
  ...
  if (!uniqueRefs.length || uniqueRefs.some((assetRef) => !/^evidence-[a-z0-9][a-z0-9_-]{2,120}$/i.test(assetRef))) throw new Error("只能处理已认领的私有营销素材。");
  ...
  for (const row of rows) {
    ...
    const token = await issueSignedToken({ pathname: row.blobKey, operations: ["get"], validUntil });
    const { presignedUrl } = await presignUrl(token, { access: "private", operation: "get", pathname: row.blobKey, validUntil, useCache: false });
    const url = new URL(presignedUrl);
    result.set(row.id, { assetRef: row.id, contentType: row.contentType, extension, hostname: url.hostname, signedGetUrl: presignedUrl });
  }
  return result;
}
