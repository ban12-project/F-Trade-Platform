...
export async function enqueueVideoJob(
  ...
  assertIdentifier(input.id, "video job ID");
  assertIdentifier(input.videoProjectId, "video project ID");
  assertIdentifier(input.modelId, "video model ID");
  assertIdentifier(input.idempotencyKey, "video idempotency key");
  assertIdentifier(input.actorId, "actor ID");
  ...
  if (!Number.isSafeInteger(input.expectedCostCents) || input.expectedCostCents < 1)
  ...
  return database.transaction(async (tx) => {
    const [created] = await tx
      ...
      .onConflictDoNothing({ target: videoJobTable.idempotencyKey })
      .returning();
    if (created) {
      await tx
        .insert(auditEvent)
        .values(auditValues("video_job.queued", created, input.actorId, now));
      return created;
    }
    const [existing] = await tx
      .select()
      .from(videoJobTable)
      .where(eq(videoJobTable.idempotencyKey, input.idempotencyKey))
      .for("update");
    if (!existing) throw new Error("Video job idempotency lookup failed");
    if (
      existing.videoProjectId !== input.videoProjectId ||
      existing.provider !== input.provider ||
      existing.modelId !== input.modelId ||
      existing.expectedCostCents !== input.expectedCostCents
    ) {
...
export async function claimConfiguredVideoJob(
  ...
  return database.transaction(async (tx) => {
    ...
    const [active] = await tx
      ...
      .where(
        and(
          ...
          eq(videoJobTable.status, "running"),
          gt(videoJobTable.leaseExpiresAt, now),
        ),
      );
    ...
    const [candidate] = await tx
      ...
      .where(
        and(
          ...
          or(
            and(
              eq(videoJobTable.status, "queued"),
              or(isNull(videoJobTable.nextAttemptAt), lte(videoJobTable.nextAttemptAt, now)),
            ),
            and(eq(videoJobTable.status, "running"), lte(videoJobTable.leaseExpiresAt, now)),
          ),
        ),
      )
      .orderBy(asc(videoJobTable.createdAt))
      .limit(1)
      .for("update");
    ...
    if (reserve > 0)
      await tx
        .update(videoProviderConfig)
        .set({
          budgetCommittedCents: sql`${videoProviderConfig.budgetCommittedCents} + ${reserve}`,
        })
        .where(eq(videoProviderConfig.provider, provider));
    const [claimed] = await tx
      .update(videoJobTable)
      .set({
        status: "running",
        attempts: sql`${videoJobTable.attempts} + 1`,
        reservedCostCents: candidate.reservedCostCents || candidate.expectedCostCents,
        claimedBy: workerId,
        claimedAt: now,
...
async function ownedRunningJob(
  jobId: string,
  workerId: string,
  ...
  const [job] = await database
    ...
    .where(
      and(
        eq(videoJobTable.id, jobId),
        eq(videoJobTable.status, "running"),
        eq(videoJobTable.claimedBy, workerId),
      ),
    )
    .for("update");
...
export async function completeVideoJob(
  ...
  assertIdentifier(resultAssetRef, "result asset reference");
  const now = new Date();
  return database.transaction(async (tx) => {
    ...
    const [completed] = await tx
      ...
      .where(eq(videoJobTable.id, job.id))
      .returning();
    if (!completed) throw new Error("Video job completion lost");
    await tx
      .insert(auditEvent)
      .values(auditValues("video_job.succeeded", completed, workerId, now));
    return completed;
  });
}
  ...
  return database.transaction(async (tx) => {
    ...
    const [failed] = await tx
      ...
      .set({
        ...
        failureCode,
        nextAttemptAt: retry ? retryAt : null,
        claimedBy: null,
        claimedAt: null,
        leaseExpiresAt: null,
      })
    ...
    await tx
      ...
      .values(
        auditValues(
          retry ? "video_job.retry_scheduled" : "video_job.failed",
          failed,
          workerId,
          now,
...
export async function cancelVideoJob(
  ...
  return database.transaction(async (tx) => {
    const [job] = await tx
      .select()
      .from(videoJobTable)
      .where(eq(videoJobTable.id, jobId))
      .for("update");
    if (!job || ["succeeded", "failed", "cancelled"].includes(job.status))
      throw new Error("Video job cannot be cancelled");
