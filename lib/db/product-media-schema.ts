...
const updatedAt = () => timestamp("updated_at", { withTimezone: true }).defaultNow().$onUpdate(() => new Date()).notNull();
...
export const productMediaAsset = pgTable(
  ...
  (table) => [
    ...
    check("product_media_origin_allowed", sql`${table.origin} in ('factory', 'user_upload', 'licensed')`),
